...
if((int)$_GET['country_id']){
	...
	if((int)$rows['zone_id']){
		...
		echo tep_draw_pull_down_menu('state', $zones_array,$state,' onchange="return get_city(this.value,checkout_confirmation,\'city\');" ');
	}else{
...
if($_POST['state']){
	...
	if((int)count($city_array)){
		echo tep_draw_pull_down_menu('city', $city_array);
	}else{
		unset($city);
