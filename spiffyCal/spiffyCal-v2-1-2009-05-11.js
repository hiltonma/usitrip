...
// GLOBAL variables
var scImgPath = './spiffyCal/images/';
...
//alert('TEST    '+dThis.getDate()+"-"+dThis.getMonth());
	...
	this.scFormatDate=function(date,format) {
		format = format+"";
		var result = "";
		var i_format = 0;
		var c = "";
		...
		var week=this.weekString;
		var w = '周'+week[date.getDay()];
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	var msDOW       = new makeArray0('&#26085;','&#19968;','&#20108;','&#19977;','&#22235;','&#20116;','&#20845;');
	...
	var scX=0; // default where to display calendar
	var scY=0;
	...
	this.disabledDate = []; //vincent add to set unselectable date
	// Name Properties
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		if (scNN6) {
			strOutput += strTemp +'<table width="185" border="1" class="cal-Table" cellspacing="0" cellpadding="0"><tr>';
		}
		else {
			strOutput += '<table width="185" border="1" class="cal-Table" cellspacing="0" cellpadding="0">'+strTemp+'<tr>';
		}
	...
	function writeControl() {
		...
		strHold='<table border="0" cellspacing="0" cellpadding="0" style="width:auto"><tr><td><input class="cal-TextBox"  name="' + this.textBoxName + '"' + strTemp + ' size="16" autocomplete="off" readonly  value="' + this.getSelectedDate() + '"></td>';
		if (!scIE) {
		...
		if ((this.focusClick==false) || (this.focusClick==true && this.hideButton==false))  {
			strHold+='<td><a class="so-BtnLink"'+strTemp;
			...
			strHold+='<img align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg +'" style="cursor: pointer; cursor: hand; margin-top:2px; margin-bottom:2px;"  width="'+ strBtnW +'" height="'+ strBtnH +'"></a></td></tr></table>';
		}else{
			strHold+='<td></td></tr></table>';
		}
	...
	function writeControl1(strTempEtra) {
		var strHold='';
		var strTemp=strTempEtra;
		var strTempMinDate='';
		var strTempMaxDate='';
		...
		strHold='<input class="cal-TextBox"  name="' + this.textBoxName + '"'  + strTemp + '  value="' + this.getSelectedDate() + '">&nbsp;&nbsp;';
		if (!scIE) {
			strTemp=' href="javascript:calClick();return false;" ';
		}
		else {
	...
	//		strHold+=this.varName+'.show();return false;">';
			...
			strHold+='<img align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg +'" style="cursor: pointer; cursor: hand;" width="'+ strBtnW +'" height="'+ strBtnH +'"></a>';
		}
		
		document.write(strHold);
	}
	
	this.writeControl1=writeControl1;
...
// put these next 2 lines in when the tiny cal btns seem to randomly disappear
	...
	function isDayValid(intWhatDay, intWhatMonth, intWhatYear){
		...
		if ((dteCur>=this.minDate) && (dteCur<=this.maxDate)) {
			//check disabled date - vincent add
			intWhatMonth = intWhatMonth+1;
			var curDateStr = intWhatYear+"-"+(intWhatMonth<10? '0'+intWhatMonth:intWhatMonth)+"-"+(intWhatDay<10? '0'+intWhatDay:intWhatDay);
			for(var i=0 ; i < this.disabledDate.length;i++){
				if(curDateStr == this.disabledDate[i]){
					return false;
				}
			}
			return true;
...
function domlay(id,trigger,lax,lay,content) {
	 ...
	 */
	// Layer visible
	lax = parseInt(lax,10) + 'px';
	lay = parseInt(lay,10) + 'px';
	if (trigger=="1"){
	...
	if (lax){
		...
		else if (document.getElementById){document.getElementById(''+id+'').style.left=lax}
		}
	// Set vertical position
	if (lay){
		if (document.layers){document.layers[''+id+''].top = lay}
		else if (document.all){document.all[''+id+''].style.top=lay}
		else if (document.getElementById){document.getElementById(''+id+'').style.top=lay}
		}
	// change content
	...
	if(isie6){
		jQuery('#spiffycalendar').append('<iframe scrolling="no" frameborder="0" style="width:189px; height:153px; top:0px; left:0px; background-color:transparent; position:absolute; z-index:-1;"></iframe>');
	}
	/*yake add end*/
