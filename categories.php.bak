...
include(DIR_FS_INCLUDES . 'javascript/' . 'webmakers_added_js.php')
?>
...
// EOF MaxiDVD: Modified For Ultimate Images Pack!
    ...
    $city_class_query = tep_db_query("select c.city_id, c.city, s.statecode from " . TABLE_TOUR_CITY . " as c ," . TABLE_TOUR_STATE . " as s where c.state_id = s.state_id and c.city !='' order by c.city ");
    while ($city_class = tep_db_fetch_array($city_class_query)) {
      $city_class_array[] = array('id' => $city_class['city_id'],
                                 'text' => $city_class['city'].', '.$city_class['statecode']);
    }
...
function generate()
{	
	<?php 	//selectez toate subcategoriile din baza de date
	
	$subquery = 'select c.*, s.statecode from '.TABLE_TOUR_CITY.'  as c ,' . TABLE_TOUR_STATE . ' as s where c.state_id = s.state_id order by c.city';	 
	$subresult = mysql_query($subquery);	
	?>
	var i; //un contor
	var nrOpt = <?php echo mysql_num_rows($subresult); ?>; //nr total de optiuni din select
	...
	for ($i = 0; $i < mysql_num_rows($subresult); $i++)
	{
		...
		echo 'optiuni['.$i.'] = new Array("'.$subrow[1].'","'.$subrow[0].'","'.$subrow[3].', '.$subrow[8].'");';
	}
	?>
      ...
      <tr>
		...
		<table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
			...
			{
			echo tep_draw_pull_down_menu('agency_id', $agency_array, $pInfo->agency_id,'id="agency_id" onChange="getInfo(\'\',\'-1\')"'); 
			}
			else
			{
			echo tep_draw_pull_down_menu('agency_id', $agency_array, $pInfo->agency_id,'id="agency_id" onChange="getInfo(\'\',\'-2\')"'); 
			}
			?>
			</td>
          </tr>
					...
					if($pInfo->products_id != "")
						...
						$countingcity = 0;
							...
							while ($destination_result = tep_db_fetch_array($destination_query))
							{
								...
								$city_edit_query = tep_db_query("select c.city_id, c.city, s.statecode from " . TABLE_TOUR_CITY . " as c ," . TABLE_TOUR_STATE . " as s where c.state_id = s.state_id and c.city_id=".$destination_result['city_id']."  order by c.city");
								$city_edit_result = tep_db_fetch_array($city_edit_query); 
								$sel2values .= "<option value=".$city_edit_result['city_id'].">".$city_edit_result['city'].', '.$city_edit_result['statecode']."</option>";
						...
						$city_new_query = tep_db_query("select ttc.city_id, ttc.city , s.statecode from " . TABLE_TOUR_CITY . " as ttc, ".TABLE_REGIONS." as regi, " . TABLE_TOUR_STATE . " as s where ttc.state_id = s.state_id and  ttc.regions_id = regi.regions_id and regi.regions_id = ".$pInfo->regions_id." and ttc.city_id not in (".$allready_product_city.") order by ttc.city");
						 while ($city_new_result = tep_db_fetch_array($city_new_query)) 
						{
						 $tempsolution .=  "<option value=".$city_new_result['city_id'].">".$city_new_result['city'].', '.$city_new_result['statecode']."</option>";
						} 
