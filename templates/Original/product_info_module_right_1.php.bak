...
<style type="text/css">
<?php  //酒店预订将使用到的 CSS代码?>
.checkDate{ border:1px solid #999; color:#223D6A; font-size:12px; height:16px; margin-top:3px; padding:2px 0 2px 5px; text-decoration: none; background:url("/image/time-selction.gif") no-repeat scroll right center transparent; width:120px;}
.cal-TextBox{ width:100px; height:16px; padding:2px 3px; background:url(/image/icons/Calendar.gif) no-repeat 89px center;}
...
if($product_info['is_hotel'] == 1 ){
	...
	$text_buy = db_to_html('立即定购');
	$text_disbale_tips = db_to_html('暂停预订，如果您需要的话，请点击右边的“恢复预订通知”按钮，输入您的E-mail地址，当恢复预订时我们会邮件通知您！');
	...
	$text_buy = db_to_html('立即定购');
	$text_disbale_tips = db_to_html('该行程已经售完，暂停预订，如果您需要的话，请点击右边的“恢复预订通知”按钮，输入您的E-mail地址，当恢复预订时我们会邮件通知您！');
}
...
<div class="buyOption">
    ...
    <div class="optionCon" >
        ...
        <div class="addCart">
              <a href="javascript:;" onclick="productSoldOut('<?php echo $product_info['products_id'];?>')" class="btnA btnABlue" title="<?php echo db_to_html('点此输入您的E-mail地址，该团恢复预订时我们将发邮件通知您！');?>"><?php echo db_to_html('恢复预订通知');?></a>
           <ul class="otherOption">       
            ...
            <li class="post" id="NewCompanion"><a href="javascript:;" class="btnBuy btnBuyPosts" onclick="showPopup('CreateNewCompanion','CreateNewCompanionCon','off','','','fixedTop','NewCompanion');"><?php echo db_to_html('发布立即结伴贴')?></a><a href="<?= tep_href_link('help_travel_companion.php');?>" target="_blank"><img title="<?php echo db_to_html('如何发布结伴贴')?>" alt="<?php echo db_to_html('如何发布结伴贴')?>" src="image/buy_tip.gif"/></a></li>
        </ul>
        </div>
     </div>
...
<div id="product_book_module" class="order margin_b10">
		...
		<div class="date">
              <div class="tit">
                <h3 class="font_bold font_size14 color_333">出团日期/价格<em class="font_size12 noBold color_b3b3b3">（点击日历上价格可以进行预订）</em></h3>
              </div>
          ...
          <ul>
			...
			if(tep_check_product_is_hotel((int)$HTTP_GET_VARS['products_id'])!=1 && $product_info['is_transfer']!=1){
				...
				if((int)$product_info['GroupBuyType']==1 || (int)$product_info['GroupBuyType']==2){
					...
					$booking_h2 = '出发时间：<a onmouseout="jQuery(\'#TimeTip\').hide();" onmouseover="jQuery(\'#TimeTip\').show();" style="color:#00388A;" href="javascript:void(0);">详细说明</a>';
					$group_time_tip = '';
					$note_info ='<p><b>说明：</b><br />&nbsp;&nbsp;&nbsp;&nbsp;在订单有效期内，您可以选择您期望的出团日期出行。购买前请详细阅读<a href="'.tep_href_link("group_buys.php","do=note").'">团购须知</a>。</p>
						...
						<p>&nbsp;&nbsp;&nbsp;&nbsp;成功购买后，您可以通过邮件或登陆“<a target="_blank" href="'.tep_href_link("account.php").'">我的八八八</a>”查看订单详情，有任何疑问请<a href="'.tep_href_link('contact_us.php').'">联系</a>我们。</p>
						<p>&nbsp;&nbsp;&nbsp;&nbsp;八八八网竭诚为您服务！</p>';
				}
				...
				if($product_info['products_id']=="147"){ 
					/*//147 团的说明//$timeText = tep_get_date_disp($last_departure_date);
					if($timeText=="01/01/2012"){
						$timeText = '2011年底';
					}*/
					$note_info = '<p><b>说明：</b><br />&nbsp;&nbsp;&nbsp;&nbsp;您可以预定八八八网提供团购优惠的有效时间段内至'.$timeText.'之前每周三或者每周六出发的任何一个出发时间作为您的出行时间，以享受此优惠！</p>
						<p>&nbsp;&nbsp;&nbsp;&nbsp;温馨提示：团购产品同时也提供酒店升级，酒店延住等服务，额外的机场接应，额外酒店接应等服务，如您有这类需求，请联系我们取得报价。</p>                      
						<p>&nbsp;&nbsp;&nbsp;&nbsp;成功购买后，您可以通过邮件或登陆“<a target="_blank" href="'.tep_href_link("account.php").'">我的八八八</a>”查看订单详情，您还可以通过手机短信查询订单详情，有任何疑问请<a href="'.tep_href_link('contact_us.php').'">联系</a>我们。</p>
						<p>&nbsp;&nbsp;&nbsp;&nbsp;八八八网竭诚为您服务！</p>';
				}
			?>
            <h2><?php echo  db_to_html($booking_h2)?><!-- <a href="javascript:;" id="ConTitleA_1"><?= db_to_html("选择")?></a> --></h2>
			  ...
			  if($product_info['products_id']=="147"){ //147 团的提示
				$buy_tips = '您可以预定在八八八网提供团购优惠的有效时间段内至'.$timeText.'之前每周三或者每周六出发的任何一个出发时间作为您的出行时间，以享受此优惠。';
			  }
			  ...
			  */
			  ...
			  <select style="<?php echo $availabletourdate_style ?>" id="availabletourdate" name="availabletourdate" title="<?php echo TEXT_SELECT_VALID_DEPARTURE_DATE?>" onchange="search_tour_end_date_ajax(&quot;<?php echo $prod_dura_day?>&quot;,this.value); document.body.focus();">
				...
				<input autocomplete="off" type="text" style="<?= $time1_display ?>width: 195px; height: 16px; border: 1px solid #999999; font-size: 12px; text-decoration: none; color:#223D6A;  margin-top:3px; padding:2px 0px 2px 5px; background:url(/image/time-selction.gif) no-repeat right center;" name="time1" id="time1" onclick="MyCalendar.SetDate(this)" value="<?= db_to_html('请选择您的出发日期')?>" onBlur="AvailabletourDate.style.display=&quot;&quot; ;AvailabletourDate.focus();AvailabletourDate.style.display=&quot;none&quot; ; search_tour_end_date_ajax(&quot;<?=$prod_dura_day?>&quot;,AvailabletourDate.value); " />
			...
			<?php /*if((int)$product_info['GroupBuyType']==2){	//限时团购团才加这个标签?>
			  ...
			  <?php if(!(int)$product_info['GroupBuyType']){ //团购团取消这个标签?>
			...
			//实现单独预订酒店
					 ...
					 <script type="text/javascript">							
							var arr_soldout_dates_hotel=Array();
							<?php
							$qry_sold_dates="SELECT products_id, products_soldout_date FROM ".TABLE_PRODUCTS_SOLDOUT_DATES." sd WHERE sd.products_id='".(int)$products_id."' UNION SELECT products_id, departure_date products_soldout_date FROM  products_remaining_seats prs WHERE prs.remaining_seats_num=0 AND prs.products_id='".(int)$products_id."'";
							$res_sold_dates=tep_db_query($qry_sold_dates);
							$ar_soldout_dates=array();
							while($row_sold_dates=tep_db_fetch_array($res_sold_dates)){
								$cntr++;
							?>
							arr_soldout_dates_hotel[<?php echo $cntr;?>] = "<?php echo $row_sold_dates['products_soldout_date'];?>";
							<?php	}		?>
						</script>
						<h2 class="hotelCheckDate"><?php echo db_to_html("入住日期："); ?></h2>
						<div class="checkDateCal"><script type="text/javascript">
								var productCheckInDate = new ctlSpiffyCalendarBox("productCheckInDate", "cart_quantity", "<?php echo $checkin_date_field_name;?>","pBtnDate","",scBTNMODE_CUSTOMBLUE);
								productCheckInDate.useDateRange = true;
								<?php  if(tep_not_null($getvalidcheckin_date)){ ?>
								productCheckInDate.minDate = new Date('<?php  echo $getvalidcheckin_date; ?>');
								<?php  } ?>
								productCheckInDate.focusClick=true;		
								productCheckInDate.readonly=true;		
								productCheckInDate.disabledDate=arr_soldout_dates_hotel;				
								productCheckInDate.JStoRunOnSelect = 'productBudget()';
								 productCheckInDate.dateFormat="MM/dd/yyyy";
								 productCheckInDate.writeControl();								 
							</script></div>	
					 </div>
					 <div class="hotelCheckDateItem" >
						<h2 class="hotelCheckDate"><?php echo db_to_html("离店日期："); ?></h2>	
						<div class="checkDateCal"><script type="text/javascript">
								var productCheckOutDate = new ctlSpiffyCalendarBox("productCheckOutDate", "cart_quantity", "<?php echo $checkout_date_field_name;?>","pBtnDate2","",scBTNMODE_CUSTOMBLUE);
								productCheckOutDate.useDateRange = true;
								<?php  if(tep_not_null($getvalidcheckin_date)){ ?>
								productCheckOutDate.minDate = new Date('<?php  echo $getvalidcheckin_date; ?>');
								<?php  } ?>
								productCheckOutDate.focusClick=true;		
								productCheckOutDate.readonly=true;						
								productCheckOutDate.JStoRunOnSelect = 'productBudget()';
								 productCheckOutDate.dateFormat="MM/dd/yyyy";
								 productCheckOutDate.writeControl();								 
							</script></div>	
					 </div>
					 </div>
			<?php		}	//}hotel-extension?>
			...
			<li id="booking_steps_3" class="bot">
              ...
              <div class="con">
                <div class="conTitle"  id="ConTitle_hot-search-params">
					...
					?>
				  <!--  <h2><?php echo $TEXT_TICKETS?><a id="SelBut3" href="javascript:;" onclick="SetPopBox('hot-search-params');"><?php echo db_to_html("选择");?></a></h2>-->
				   <h2><?php echo $TEXT_TICKETS?><a id="ConTitleA_hot-search-params"  href="javascript:;" ><?php echo db_to_html("选择");?></a></h2>
					<?php
					//选择房间选项 start
...
if(jQuery.browser.msie && (parseInt(jQuery.browser.version) <= 7)){
	...
	if(jQuery("#OptionsWrap").height() < minHeight){
		...
		<div class="priceNull"  id="PriceNull">
		<?php
        ...
        <div class="price"  id="Price" style="display:none">
            <p><label><?php echo db_to_html("(请以我们与您确认的最终价格为准)")?><br /></label><label><br><?php echo db_to_html("美元")?></label></p>
            <p class="us" id="currecy_display_usd">0.00</p>
		...
		?>
		 ...
		 ?>
		 	...
		 	<a  href="javascript:;"  id="check_remaining_seats_cart"   onclick="AddToCart();" title="<?php echo db_to_html('放入购物车')?>"  alt="<?php echo db_to_html('放入购物车')?>" class="addCartBtn <?php if($language == 'tchinese')echo ' addCartBtnTchinese';?> "  ><?php echo db_to_html('放入购物车')?></a>
			  ...
			  <a href="javascript:;" id="check_remaining_seats_buy" class="btnBuy btnBuyBook" onclick="return SubmitCartQuantityFrom()" onmousemove="check_remaining_seats()"><ins><button type="button"><?= db_to_html('立即定购');?></button></ins></a>
			  </div>--> 
		  <?php
		  }
