...
$payment_modules->update_status();
  ...
  //恢复order->info[total]
  $total = 0;foreach($order->products as $product)$total+= $product['final_price'];
  $order->info['total'] = $total;
  $order->info['subtotal'] = $total;
  $order->info['payment_method'] = $payment_name;
...
//if ( ( is_array($payment_modules->modules) && (sizeof($payment_modules->modules) > 1) && !is_object($$payment) ) || (is_object($$payment) && ($$payment->enabled == false)) ) {
  if ( (is_array($payment_modules->modules)) && (sizeof($payment_modules->modules) > 1) && (!is_object($$payment)) && (!$credit_covers) && (!$customer_shopping_points_spending) ) {
	 tep_redirect(tep_href_link('account_history_payment_method.php', 'error_message=' . urlencode(ERROR_NO_PAYMENT_MODULE_SELECTED), 'SSL'));
  }	 
########  Points/Rewards Module V2.1rc2a EOF #################*/
  ...
  /*if ( (is_array($payment_modules->modules)) && (sizeof($payment_modules->modules) > 1) && (!is_object($$payment)) && (!$credit_covers) ) {
	...
	//设置错误返回页面
	$error_back_page = 'account_history_payment_method.php';
		  ...
		  if ($error == false) {
			...
			if (ACCOUNT_STATE == 'true') {
				...
				if((int)$get_row['zone_id']){
					$sql_data_array['entry_state'] = '';$sql_data_array['entry_zone_id'] = (int)$get_row['zone_id'];
				}else{
					$sql_data_array['entry_state'] = ($state);$sql_data_array['entry_zone_id'] = '0';
				}
				/*howard added end*/
			}			
...
if(isset($_POST['shipto']) && $error == false){
	...
	if (ACCOUNT_STATE == 'true') {
		...
		if((int)$get_row['zone_id']){
			...
			$sql_data_array['entry_state'] = html_to_db($ship_state);$sql_data_array['entry_zone_id'] = '0';
		}
	}
	...
	if((int)$_POST['shipto']){	
		...
		if(!tep_session_is_registered('customer_default_ship_address_id')){tep_session_register('customer_default_ship_address_id');}		
	...
	// howard fixed 
	$customers_telephone = '';$customers_fax = '';$customers_mobile_phone = '';
...
/*add or update ship address - end}*/  
  ...
  $breadcrumb->add(db_to_html('订单资讯'), tep_href_link('account_history_info.php', 'order_id='.$_POST['order_id'], 'SSL'));
  $breadcrumb->add(db_to_html('修改付款方式'),tep_href_link('account_history_payment_method.php', 'order_id='.$_POST['order_id'], 'SSL'));
  $breadcrumb->add(db_to_html('确认付款'));
  $validation_include_js = 'true';
  $is_my_account = true;  
  $content = 'account_history_payment_checkout';  
