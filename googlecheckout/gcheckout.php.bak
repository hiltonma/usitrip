...
for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
	...
	$gcheck->push('item');
	$gcheck->element('merchant-item-id', (int)$products[$i]['id']);
	...
	$gcheck->push('merchant-private-item-data');
	//amit added to fixed extra information start
		 $arrival_date = "";
		 $departure_date = "";
		...
		$gcheck->element('products-departure-date', htmlentities($products[$i]['dateattributes'][0]));
		$gcheck->element('products-departure-time', htmlentities($products[$i]['dateattributes'][1]));
		$gcheck->element('products-departure-location', htmlentities($products[$i]['dateattributes'][2]));
		$gcheck->element('products-room-price', htmlentities($products[$i]['roomattributes'][0]));
		$gcheck->element('products-room-info', htmlentities($products[$i]['roomattributes'][1]));
		$gcheck->element('airline-name', htmlentities($order->info['airline_name'][$i]));
		$gcheck->element('flight-no', htmlentities($order->info['flight_no'][$i]));
		$gcheck->element('airline-name-departure', htmlentities($order->info['airline_name_departure'][$i]));
		$gcheck->element('flight-no-departure', htmlentities($order->info['flight_no_departure'][$i]));
		$gcheck->element('airport-name', htmlentities($order->info['airport_name'][$i]));
		$gcheck->element('airport-name-departure', htmlentities($order->info['airport_name_departure'][$i]));
		$gcheck->element('arrival-date', htmlentities($arrival_date));
		$gcheck->element('arrival-time', htmlentities($order->info['arrival_time'][$i]));
		$gcheck->element('departure-date', htmlentities($departure_date));
		$gcheck->element('departure-time', htmlentities($order->info['departure_time'][$i]));
		$gcheck->element('guest-name', htmlentities($guest_name));
		if(!empty($guest_body_weight))
		$gcheck->element('guest-body-weight', htmlentities($guest_body_weight));
		$gcheck->element('depature-full-address', htmlentities($depature_full_address));
		//amit added to fixed extra information end 
		$gcheck->pop('merchant-private-item-data');
		$gcheck->pop('item'); 
}
$gcheck->pop('items');
...
*/
		...
		$gcheck->push('flat-rate-shipping', array ('name' => htmlentities($order->info['shipping_method'])));
		$gcheck->element('price', number_format($order->info['shipping_cost'], 2, '.', ''), array ('currency' => 'USD'));
		$gcheck->pop('flat-rate-shipping');
...
<table  border="0" width="100%" class="table-1" cellspacing="12" cellpadding="5"> 
  ...
  <tr><td align="center" valign="middle" class = "main">
    <p>	
	<form method="POST" name="google_payment_info" action="<?php echo $current_checkout_url; ?>">
...
<script>
document.google_payment_info.submit();
</script>
<!-- ** END GOOGLE CHECKOUT ** -->