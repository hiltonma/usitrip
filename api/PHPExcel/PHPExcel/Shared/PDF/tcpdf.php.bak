	...
	class TCPDF {
		...
		*/
		protected function _parsejpeg($file) {
			...
			if ((!isset($a_price['channels'])) OR ($a_price['channels'] == 3)) {
				$colspace = 'DeviceRGB';
			} elseif ($a_price['channels'] == 4) {
				$colspace = 'DeviceCMYK';
			} else {
				$colspace = 'DeviceGray';
			}
			$bpc = isset($a_price['bits']) ? $a_price['bits'] : 8;
			$data = file_get_contents($file);
			return array('w' => $a_price[0], 'h' => $a_price[1], 'cs' => $colspace, 'bpc' => $bpc, 'f' => 'DCTDecode', 'data' => $data);
		}
					...
					if ($element{0} == '/') {
						...
						if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
							...
							for ($i = ($dom[$key]['parent'] + 1); $i < $key; ++$i) {
								$dom[($dom[$key]['parent'])]['content'] .= $a_price[$dom[$i]['elkey']];
							}
							$key = $i;
						}
						...
						if (($dom[$key]['value'] == 'tr') AND ($dom[($dom[$key]['parent'])]['thead'] === true)) {
							if ($this->empty_string($dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'])) {
								$dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'] = $a_price[$dom[($dom[($dom[$key]['parent'])]['parent'])]['elkey']];
							}
							for ($i = $dom[$key]['parent']; $i <= $key; ++$i) {
								$dom[($dom[($dom[$key]['parent'])]['parent'])]['thead'] .= $a_price[$dom[$i]['elkey']];
							}
						}
