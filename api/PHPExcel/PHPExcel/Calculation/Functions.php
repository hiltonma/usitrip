...
//		$rMax			= self::flattenSingleValue($rMax);
	...
	private static function _inverse_ncdf($p) {
		...
		if (0 < $p && $p < $p_low) {
			...
			$q = $p - 0.5;
			$r = $q * $q;
			return ((((($a_price[1] * $r + $a_price[2]) * $r + $a_price[3]) * $r + $a_price[4]) * $r + $a_price[5]) * $r + $a_price[6]) * $q /
...
//			$fRet = sqrt(0.636619772 / $fNum) * sin($fNum - 2.356194491);
	...
	private static function _vlookupSort($a,$b) {
		...
		if (strtolower($a_price[$firstColumn]) == strtolower($b[$firstColumn])) {
			return 0;
		}
		return (strtolower($a_price[$firstColumn]) < strtolower($b[$firstColumn])) ? -1 : 1;
