...
{
	$region_combo .= "<option value='".$i."'>".db_to_html($result['departure_region'])."</option>";	//所在区域列表
...
function populatecombo2(elem, index, addr, city){
	...
	if (array1.length >= index){
		if (array1[index]){
			...
			elem.options[elem.options.length] = new Option("<?php echo db_to_html('[没有可用信息]');?>", 0);
		}
	}
	else{
		elem.options[elem.options.length] = new Option("<?php echo db_to_html('[没有可用信息]');?>", 0);
...
<tr> 
		...
		<SELECT NAME="_1_H_hotel2" class="sel3"  SIZE="0" ONCHANGE="return(clickcombo('2',document.cart_quantity._1_H_hotel1,document.cart_quantity._1_H_hotel2, document.cart_quantity._1_H_hotel3, document.cart_quantity._1_H_address,document.cart_quantity._1_H_city,5));"><OPTION><?php echo TEXT_SELECT_NOTHING;?></OPTION></SELECT>
		<SELECT NAME="_1_H_hotel3" class="sel3" SIZE="0" style="display:none"><OPTION><?php echo TEXT_SELECT_NOTHING;?></OPTION></SELECT><br>
...
<tr>
		<td width="1">&nbsp;</td>
		<td>&nbsp;</td>
		<td class="buy_options_title" valign="top"><b><?php echo TEXT_PICKUP_ADDRESS;?></b></td>
</tr>
<tr>
		<td width="1">&nbsp;</td>
		<td>&nbsp;</td>
			<td class="main" valign="top"><div id="fulladdress"></div><div id="mappath" style="display:none"></div>
			<input type="text" class="validate-select-custom-pickup " id="_1_H_address" title="<?php echo TEXT_SELECT_PICKUP_LOCATION; ?>" name="_1_H_address" style="border:0px; height:0px; width:0px;" /><?php // size="30" ?>					
