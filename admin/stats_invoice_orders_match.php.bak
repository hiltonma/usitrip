...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('stats_invoice_orders_match');
...
if($HTTP_GET_VARS['action'] == 'edit_invo_proccess'){
	$display_msg_txt = '';
	if($HTTP_POST_VARS['op_id'] != '' && ($HTTP_POST_VARS['inv_no'] != $HTTP_POST_VARS['prev_inv_no'] || $HTTP_POST_VARS['inv_amt'] != $HTTP_POST_VARS['prev_inv_amt'] )  ){
	
	 /* update code start */
	 $orders_products_id = $HTTP_POST_VARS['op_id'];
	 $get_ord_model_info_array = tep_get_products_name_model_from_orders_products($orders_products_id);
	 
	     $adjusted_retail_price = 0;
		 $adjusted_cost_price = 0;
		 
		 //$total_invoices_cost_retail_comments = stripslashes($products_details["invoice_comments"]).'!!###!!'.stripslashes($products_details["invoice_comments_retail"]);
		  
		 $sql_data_array = array(			  
			'orders_products_id' => $orders_products_id,
			'products_model' => $get_ord_model_info_array["products_model"],
			'products_name' => str_replace("'", "&#39;", $get_ord_model_info_array["products_name"]),			
			'retail' => $adjusted_retail_price,	
			'cost' => $adjusted_cost_price,	
			'invoice_number' => $HTTP_POST_VARS['inv_no'],	
			'invoice_amount' => $HTTP_POST_VARS['inv_amt'],
			'updated_by' => $login_id,
	 ...
	 $display_msg_txt .= "Invoice No. and Amt updated successfully. please close this window and refresh parent window to see updated effect";
	}else {
...
if($HTTP_GET_VARS['action'] == 'show_popup_form' && $HTTP_GET_VARS['op_id'] != ''){
	//fetch invoice no and mat from order product id start
	$get_ord_model_info_array = tep_get_products_name_model_from_orders_products($HTTP_GET_VARS['op_id']);
	//fetch invoice no and mat from order product id end

	?>
	<table width="100%" cellpadding="0" cellspacing="0">
	<tr><td>
		<?php echo tep_draw_form('search', FILENAME_STATS_INVOICE_ORDER_MATCH, 'action=edit_invo_proccess', 'post'); ?>
		<table border="0" cellspacing="3" cellpadding="3">
		  <tr>
			<td class="main">Invoice No.</td>
			<td><input type="text" name="inv_no" value="<?php echo $get_ord_model_info_array['invoice_number']; ?>"><input type="hidden" name="prev_inv_no" value="<?php echo $get_ord_model_info_array['invoice_number']; ?>"></td>
		  </tr>
		  <tr>
			<td class="main">Invoice Amount</td>
			<td>
			<input type="text" name="inv_amt" value="<?php echo $get_ord_model_info_array['invoice_amount']; ?>">
			<input type="hidden" name="prev_inv_amt" value="<?php echo $get_ord_model_info_array['invoice_amount']; ?>">
			<input type="hidden" name="op_id" value="<?php echo (int)$HTTP_GET_VARS['op_id'];?>">
			</td>
		  </tr>
	...
	foreach ($_POST['prev_orderitemid'] as $orderitemid=>$status){
		 ...
		 else
		 {
			...
			tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array_locations, 'update', " orders_products_id = '".$orderitemid."' ");					
				...
				tep_redirect(tep_href_link(FILENAME_STATS_INVOICE_ORDER_MATCH, tep_get_all_get_params(array('action','oID','products_id')).'action=view_report'));
...
//echo $login_id;
include DIR_FS_CLASSES . 'Remark.class.php';
$listrs = new Remark('stats_invoice_orders_match');
...
<?
	...
	if(!isset($HTTP_GET_VARS['action']) && !isset($HTTP_GET_VARS['dept_start_date']) && !isset($HTTP_GET_VARS['order_date_purchased_to'])){		
	  ...
	  $_GET['dept_end_date'] = $HTTP_GET_VARS['dept_end_date'] = date('m/t/Y');	  
	}
?>
...
<table border="0" width='100%' cellspacing="0" cellpadding="2">
						<tr>
							...
							<td colspan="9" valign="top">
								<?php echo tep_draw_form('search', FILENAME_STATS_INVOICE_ORDER_MATCH, '', 'get'); ?>
...
//--></script>
						...
						<?php echo tep_draw_form('orderitemcheck', FILENAME_STATS_INVOICE_ORDER_MATCH, tep_get_all_get_params(array('selected_box','action')).'action=proccess_payment', 'post','onSubmit="return confirm(\'Are you sure you want to check selected orders for this report?\');"'); ?>
						<tr>
						...
						<tr class="dataTableHeadingRow">
							...
							<?php /*?><td class="dataTableHeadingContent">Tours</td>
							<td class="dataTableHeadingContent">Provider</td><?php */?>
							...
							<?php /*?><td class="dataTableHeadingContent">Gross Profit</td>
							<td class="dataTableHeadingContent">Gross Profit<br>(%)</td>
							<td class="dataTableHeadingContent">Paid</td>
							<td class="dataTableHeadingContent">Payment History</td><?php */?>
						</tr>
				...
				/////   end - search coding    ///////
						...
						$prods_query_sql = "select p.agency_id, p.provider_tour_code, o.orders_id, o.orders_status, o.date_purchased, op.orders_products_id, op.payment_paid, op.products_id, op.products_model,op.payment_adjusted_blocked, op.products_name, op.final_price, op.products_quantity, op.final_price_cost,op.products_departure_date, op.orders_products_id, oph.cost, oph.retail, oph.last_updated_date, oph.invoice_number, oph.invoice_amount, oph.updated_by, oph.comment  from   ".TABLE_ORDERS_PRODUCTS." as op, ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." oph LEFT JOIN ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." oph2 ON oph.orders_products_id = oph2.orders_products_id
AND oph.ord_prod_history_id < oph2.ord_prod_history_id and oph.last_updated_date < oph2.last_updated_date,  orders as o, products as p where oph2.last_updated_date IS NULL and ( o.orders_status = '100006' or o.orders_status = '100005' ) ".$where."  and oph.orders_products_id = op.orders_products_id  and op.orders_id = o.orders_id and  op.products_id=p.products_id group by op.orders_products_id ".$sortorder." "; //o.last_modified desc
							 ...
							 while ($prods = tep_db_fetch_array($prods_query)) {							
									...
									<?php /*?><td class="dataTableContent"><?php echo '<a target="_blank" href="' . tep_catalog_href_link(FILENAME_PRODUCT_INFO, 'products_id=' .$prods['products_id']) . '">' . $prods['products_name'].'</a>' .' [<a target="_blank" href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . tep_get_products_catagory_id($prods['products_id']) . '&pID=' . $prods['products_id'].'&action=new_product') . '">' . $prods['products_model'] . '</a>'.']'.' [<a target="_blank" href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . tep_get_products_catagory_id($prods['products_id']) . '&pID=' . $prods['products_id'].'&action=new_product') . '">' . $prods['provider_tour_code'] . '</a>'.']'; //&tabedit=eticket ?></td>
									<td class="dataTableContent"><?= tep_get_travel_agency_name($prods['agency_id']); ?></td><?php */?>
									
									<td class="dataTableContent" nowrap="nowrap"><?= $prods['invoice_number']; ?> &nbsp;
									<?php if($access_full_edit == 'true') { ?>
									<a href="javascript:popupImageWindow('<?php echo tep_href_link(FILENAME_STATS_INVOICE_ORDER_MATCH, 'action=show_popup_form&op_id='.$prods['orders_products_id']); ?>');">[<u><font color="#0000CC">Edit</font></u>]<?php //echo tep_image(DIR_WS_ICONS . 'edit.gif', 'Add/Edit Invoice No. or Amount');?></a>
									<?php } ?>
									</td>			
									...
									<?php /*?><td class="dataTableContent">$<?= $gross_profit;?></td>
									...
									echo '<a  target="_blank" href="' . tep_href_link(FILENAME_STATS_PAID_PAYMENT_ORDER_HISTORY,'payment_id='.$select_check_order_product_payment_history['ord_prod_payment_id'].'&item_id='.$prods['orders_products_id'], 'NONSSL') . '"><strong>View';									
									if($view_count>1){
										echo $view_count;
									}
									echo '</strong></a>'; }else { echo '&nbsp;';} ?></td><?php */?>
									</tr>						
