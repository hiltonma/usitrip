...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('stats_invoice_amount_mismatch');
	...
	foreach ($_POST['prev_orderitemid'] as $orderitemid=>$status){
		 ...
		 else
		 {
			...
			tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array_locations, 'update', " orders_products_id = '".$orderitemid."' ");					
				...
				tep_redirect(tep_href_link(FILENAME_STATS_INVOICE_AMOUNT_MISMATCH, tep_get_all_get_params(array('action','oID','products_id')).'action=view_report'));
...
//echo $login_id;
include DIR_FS_CLASSES . 'Remark.class.php';
$listrs = new Remark('stats_invoice_amount_mismatch');
...
<table border="0" width='100%' cellspacing="0" cellpadding="2">
						<tr>
							...
							<td colspan="9" valign="top">
								<?php echo tep_draw_form('search', FILENAME_STATS_INVOICE_AMOUNT_MISMATCH, '', 'get'); ?>
...
//--></script>
						...
						<?php echo tep_draw_form('orderitemcheck', FILENAME_STATS_INVOICE_AMOUNT_MISMATCH, tep_get_all_get_params(array('selected_box','action')).'action=proccess_payment', 'post','onSubmit="return confirm(\'Are you sure you want to check selected orders for this report?\');"'); ?>
...
AND oph.ord_prod_history_id < oph2.ord_prod_history_id and oph.last_updated_date < oph2.last_updated_date,  orders as o, products as p where oph2.last_updated_date IS NULL and oph.invoice_amount <> op.final_price_cost  and oph.invoice_amount > 0  ".$where."  and oph.orders_products_id = op.orders_products_id  and op.orders_id = o.orders_id and  op.products_id=p.products_id group by op.orders_products_id ".$sortorder." "; //o.last_modified desc
