...
if ($HTTP_GET_VARS['action']) {
	...
	case 'save':
		...
		tep_redirect(tep_href_link(FILENAME_FAQDESK_CONFIGURATION, 'gID=' . $HTTP_GET_VARS['gID'] . '&cID=' . $cID));
		break;
	}
}

$cfg_group_query = tep_db_query(
"select configuration_group_title from " . TABLE_FAQDESK_CONFIGURATION_GROUP . " where configuration_group_id = '" . $HTTP_GET_VARS['gID'] . "'"
);
...
<?php
 ...
 "select configuration_id, configuration_title, configuration_value, use_function from " . TABLE_FAQDESK_CONFIGURATION . " where configuration_group_id = '" . $HTTP_GET_VARS['gID'] . "' order by sort_order"
);
...
"select configuration_key, configuration_description, date_added, last_modified, use_function, set_function from " . TABLE_FAQDESK_CONFIGURATION . " where configuration_id = '" . $configuration['configuration_id'] . "'"
);
	...
	if ( (is_object($cInfo)) && ($configuration['configuration_id'] == $cInfo->configuration_id) ) {
		...
		. tep_href_link(FILENAME_FAQDESK_CONFIGURATION, 'gID=' . $HTTP_GET_VARS['gID'] . '&cID=' . $cInfo->configuration_id . '&action=edit')
		. '\'">' . "\n";
	} else {
...
if ( (is_object($cInfo)) && ($configuration['configuration_id'] == $cInfo->configuration_id) ) {
	...
	echo '<a href="' . tep_href_link(FILENAME_FAQDESK_CONFIGURATION, 'gID=' . $HTTP_GET_VARS['gID'] . '&cID='
...
case 'edit':
	...
	$contents = array(
	'form' => tep_draw_form('configuration', FILENAME_FAQDESK_CONFIGURATION, 'gID=' . $HTTP_GET_VARS['gID'] . '&cID=' . $cInfo->configuration_id
	. '&action=save')
	);
	...
	$contents[] = array(
		...
		. tep_href_link(FILENAME_FAQDESK_CONFIGURATION, 'gID=' . $HTTP_GET_VARS['gID'] . '&cID=' . $cInfo->configuration_id)
...
if (is_object($cInfo)) {
	...
	$contents[] = array(
		'align' => 'center',
		'text' => '<a href="' . tep_href_link(FILENAME_FAQDESK_CONFIGURATION, 'gID=' . $HTTP_GET_VARS['gID']
