...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('newsdesk_reviews');
...
if ($HTTP_GET_VARS['action']) {
	...
	case 'update':
		...
		tep_redirect(tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $reviews_id));
		break;
	case 'deleteconfirm':
		$reviews_id = tep_db_prepare_input($HTTP_GET_VARS['rID']);
		...
		tep_redirect(tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page']));
		break;
...
//echo $login_id;
include DIR_FS_CLASSES . 'Remark.class.php';
$listrs = new Remark('newsdesk_reviews');
...
?>
	...
	<tr>
<?php echo tep_draw_form('review', FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $HTTP_GET_VARS['rID'] . '&action=preview'); ?>
		<td>
...
. tep_image_submit('button_preview.gif', IMAGE_PREVIEW) . ' <a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page='
...
<?php
	...
	if ($HTTP_POST_VARS) {
		...
		$reviews_query = tep_db_query("select r.reviews_id, r.newsdesk_id, r.customers_name, r.date_added, r.last_modified, r.reviews_read, rd.reviews_text, r.reviews_rating from " . TABLE_NEWSDESK_REVIEWS . " r, " . TABLE_NEWSDESK_REVIEWS_DESCRIPTION . " rd where r.reviews_id = '" . $HTTP_GET_VARS['rID'] . "' and r.reviews_id = rd.reviews_id");
...
?>
	...
	<tr>
<?php echo tep_draw_form('update', FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $HTTP_GET_VARS['rID'] . '&action=update', 'post', 'enctype="multipart/form-data"'); ?>
		<td>
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr>
		<td class="main" valign="top">
...
<?php echo '<a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a> ' . tep_image_submit('button_update.gif', IMAGE_UPDATE) . ' <a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?>
		</td>
</form>
	</tr>
...
while ($reviews = tep_db_fetch_array($reviews_query)) {
	...
	if ( (is_object($rInfo)) && ($reviews['reviews_id'] == $rInfo->reviews_id) ) {
		echo '<tr class="dataTableRowSelected" onmouseover="this.style.cursor=\'hand\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=preview') . '\'">' . "\n";
	} else {
		echo '<tr class="dataTableRow" onmouseover="this.className=\'dataTableRowOver\';this.style.cursor=\'hand\'" onmouseout="this.className=\'dataTableRow\'" onclick="document.location.href=\'' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $reviews['reviews_id']) . '\'">' . "\n";
	}
?>

		<td class="dataTableContent">
<?php echo '<a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $reviews['reviews_id'] . '&action=preview') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . newsdesk_get_products_name($reviews['newsdesk_id']); ?>
		</td>
...
<?php if ( (is_object($rInfo)) && ($reviews['reviews_id'] == $rInfo->reviews_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $reviews['reviews_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>
		&nbsp;
		</td>
	</tr>
...
$contents = array();
	...
	case 'delete':
		...
		$contents = array('form' => tep_draw_form('reviews', FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=deleteconfirm'));
		$contents[] = array('text' => TEXT_INFO_DELETE_REVIEW_INTRO);
		$contents[] = array('text' => '<br><b>' . $rInfo->newsdesk_article_name . '</b>');
		$contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . ' <a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
		break;
	default:
	if (is_object($rInfo)) {
		$heading[] = array('text' => '<b>' . $rInfo->newsdesk_article_name . '</b>');

		$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=edit') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');

if ($rInfo->approved==0){
	$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=approve_review') . '">' . tep_image_button('button_review_approve.gif', TEXT_APPROVE) . '</a>');
}
elseif ($rInfo->approved==1) {
	$contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_NEWSDESK_REVIEWS, 'page=' . $HTTP_GET_VARS['page'] . '&rID=' . $rInfo->reviews_id . '&action=disapprove_review') . '">' . tep_image_button('button_review_disapprove.gif', TEXT_DISAPPROVE) . '</a>');
}
else {
