...
. $HTTP_GET_VARS['pID'] . "' and p.faqdesk_id = pd.faqdesk_id and pd.language_id = '" . $languages_id . "'");
$product = tep_db_fetch_array($product_query);
...
<?php echo tep_draw_hidden_field('faqdesk_date_added', (($pInfo->faqdesk_date_added) ? $pInfo->faqdesk_date_added : date('Y-m-d'))) . tep_draw_hidden_field('faqdesk_date_available', (($pInfo->faqdesk_date_available) ? $pInfo->faqdesk_date_available : date('Y-m-d'))) . tep_image_submit('button_preview.gif', IMAGE_PREVIEW) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_FAQDESK, 'cPath=' . $cPath . '&pID=' . $HTTP_GET_VARS['pID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>'; ?>
		</td>
	</tr>
</table>
...
$faqdesk_answer_short[1] = nl2br($HTTP_POST_VARS['faqdesk_answer_short_1']);
$faqdesk_answer_short[2] = nl2br($HTTP_POST_VARS['faqdesk_answer_short_2']);
$faqdesk_answer_short[3] = nl2br($HTTP_POST_VARS['faqdesk_answer_short_3']);
$faqdesk_answer_short[4] = nl2br($HTTP_POST_VARS['faqdesk_answer_short_4']);

$faqdesk_answer_long[1] = nl2br($HTTP_POST_VARS['faqdesk_answer_long_1']);
$faqdesk_answer_long[2] = nl2br($HTTP_POST_VARS['faqdesk_answer_long_2']);
$faqdesk_answer_long[3] = nl2br($HTTP_POST_VARS['faqdesk_answer_long_3']);
$faqdesk_answer_long[4] = nl2br($HTTP_POST_VARS['faqdesk_answer_long_4']);
...
$languages = tep_get_languages();
for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
	echo tep_draw_hidden_field('faqdesk_question[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_question[$languages[$i]['id']])));
	echo tep_draw_hidden_field('faqdesk_answer_long[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_answer_long[$languages[$i]['id']])));
	echo tep_draw_hidden_field('faqdesk_answer_short[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_answer_short[$languages[$i]['id']])));
	echo tep_draw_hidden_field('faqdesk_extra_url[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_extra_url[$languages[$i]['id']])));
	echo tep_draw_hidden_field('faqdesk_image_text[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_image_text[$languages[$i]['id']])));
	echo tep_draw_hidden_field('faqdesk_image_text_two[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_image_text_two[$languages[$i]['id']])));
	echo tep_draw_hidden_field('faqdesk_image_text_three[' . $languages[$i]['id'] . ']', tep_htmlspecialchars(stripslashes($faqdesk_image_text_three[$languages[$i]['id']])));
}
