...
<!-- body_text //-->
    <td width="100%" valign="top">
      <form action="<?php echo FILENAME_DBMTCE_ANALYZE; ?>" method="post">
      <input type=hidden name="action" value="doRepair">
      <table border="0" align="center" width="100%" cellspacing="0" cellpadding="2">
        <tr><td class="pageHeading"><?php echo TEXT_HEADING_DBANALYZE; ?></td></tr>
        <tr><td class="main"><br><?php echo TEXT_DISPLAY_DBANALYZE; ?><br>&nbsp;</td></tr>
        <tr>
          <td align="center">
            <select name="tables[]" size="10" MULTIPLE>
              <option value="all" selected><?php echo TEXT_DISPLAY_DBANALYZE_ALLTABLES; ?>
<?php
...
?>
        ...
        <tr>
          <td align="center">&nbsp;<br><input type=submit value="<?php echo TEXT_DISPLAY_DBANALYZE_SUBMIT; ?>"></td>
        </tr>
      </table>
      </form>
<?php
  if ($HTTP_POST_VARS['action']=="doRepair" && $HTTP_POST_VARS['tables'][0]) {
    ...
    $toAnalyze = SUBSTR($toAnalyze,0,-1);
    echo "      <table border=\"0\" width=\"100%\" cellspacing=\"0\" cellpadding=\"2\">
        <tr><td colspan=\"2\" class=\"pageHeading\">" . TEXT_HEADING_DBANALYZE_RESULTS . "<br>&nbsp;</td></tr>";
    $tables_query = tep_db_query('ANALYZE TABLE ' . $toAnalyze);
    while ($tables = tep_db_fetch_array($tables_query)) {
