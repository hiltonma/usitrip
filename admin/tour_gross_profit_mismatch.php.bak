...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('tour_gross_profit_mismatch');
		 ...
		 else
		 {
		   ...
		   /*$sql_data_array_locations = array(		   
			...
			tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array_locations, 'update', " orders_products_id = '".$orderitemid."' ");					  */
				...
				tep_redirect(tep_href_link(FILENAME_TOUR_GROSS_PROFIT_MISMATCH, tep_get_all_get_params(array('action','oID','products_id')).'action=view_report'));
...
//echo $login_id;
include DIR_FS_CLASSES . 'Remark.class.php';
$listrs = new Remark('tour_gross_profit_mismatch');
...
<table border="0" width='100%' cellspacing="0" cellpadding="2">
						<tr>
							...
							<td colspan="9" valign="top">
								<?php echo tep_draw_form('search', FILENAME_TOUR_GROSS_PROFIT_MISMATCH, '', 'get'); ?>
...
//--></script>
						...
						<?php echo tep_draw_form('orderitemcheck', FILENAME_TOUR_GROSS_PROFIT_MISMATCH, tep_get_all_get_params(array('selected_box','action')).'action=proccess_payment_cofirm', 'post','onSubmit="return confirm(\'Are you sure you want to check selected orders for this report?\');"'); ?>
				...
				/////   end - search coding    ///////
					...
					$items_sold_cnt =0;
					  ...
					  $prods_query_sql = "select o.last_modified, p.agency_id, p.provider_tour_code,p.products_margin, o.orders_id, o.orders_status, o.date_purchased, op.orders_products_id, op.payment_paid, op.products_id, op.products_model,op.payment_adjusted_blocked, op.products_name, op.final_price, op.products_quantity, op.final_price_cost,op.products_departure_date, op.orders_products_id, oph.cost, oph.retail, oph.last_updated_date, oph.invoice_number, oph.invoice_amount, oph.updated_by, oph.comment from   ".TABLE_ORDERS_PRODUCTS." as op, ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." as oph, orders as o, products as p where oph.orders_products_id = op.orders_products_id
								 ".$where." and o.orders_status<>6 and p.products_margin > round((((op.final_price-op.final_price_cost)/op.final_price)*100),0) and op.orders_id = o.orders_id and  op.products_id=p.products_id group by oph.orders_products_id order by o.last_modified desc";
