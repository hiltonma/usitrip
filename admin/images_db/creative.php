...
if(tep_not_null($_GET['search_type']) && tep_not_null($_GET['keyword'])){
	switch($_GET['search_type']){
		case 'title': $where.=" AND creative_name regexp '".tep_db_prepare_input($_GET['keyword'])."' "; break;
		case 'description': $where.=" AND creative_description regexp '".tep_db_prepare_input($_GET['keyword'])."' "; break;
		case 'upload_people': $where.=" AND creative_upload_people regexp '".tep_db_prepare_input($_GET['keyword'])."' "; break;
		//case 'creative_date': $where.=" AND creative_date regexp '".tep_db_prepare_input($_GET['keyword'])."' "; break;
	}
}
...
if(tep_not_null($_GET['min_date'])){
	$where .= ' AND creative_date >= "'.$_GET['min_date'].' 00:00:00" ';
}
if(tep_not_null($_GET['max_date'])){
	$where .= ' AND creative_date <= "'.$_GET['max_date'].' 23:59:59" ';
}


//排序
$order_by = " ORDER BY creative_date DESC ";
...
//最大记录数
	...
	$sql = tep_db_query('SELECT *  FROM `creative` '.$where.$order_by.$limit);	
	$row = tep_db_fetch_array($sql);

	//总记录
	
		$sql_total = tep_db_query("select count(*) as c_num from `creative` $where ");
...
function submit_reviews(from_name, Layer_id){
	...
	var creative_id = from_name.replace(/FormWriteReviews/,"");
	var Submit_id = "Submit_" + creative_id;
	...
	AJAX.onreadystatechange = function() { 
		if (AJAX.readyState == 4 && AJAX.status == 200 ) { 
			if(AJAX.responseText.search(/\[OK\]1\[\/OK\]/g)!=-1){		
				...
				var Layer_look_reviews = "Layer_look_reviews_"+creative_id;
				LookReviews(Layer_look_reviews,creative_id);
...
function LookReviews(id,reviews_type_id){
	...
	if(ajax_status==true){
		...
		var post_str="reviews_type=creative&reviews_type_id=" + reviews_type_id;
		id.className = "Layer";
...
var creative_num = 0;
function auto_load_creative_for_relevant(ImagesSrc){
	preloadImages[creative_num] = new Image();
	preloadImages[creative_num].src = ImagesSrc;
	creative_num++;
}
...
<table border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td>
	<fieldset style="background-image:url(creative/bg.gif)">
  ...
  <tr>
    ...
    <td align="right"><b><?php echo DesignDraft_Database?></b></td>
    <td align="right">&nbsp;</td>
  </tr>
</table>
	</fieldset>	</td>
...
<?php
	...
	//$option_array[3]['id']='creative_date';
...
<?php
 	$sql_group = tep_db_query('SELECT *  FROM `images_group` where group_name!="设计参考" AND group_name!="店铺风格参考" ORDER BY group_id ASC');	
...
?>		</td>
        ...
        <input name="Submit" type="button" onclick="MM_goToURL('parent','upload_creative.php');return document.MM_returnValue" value="<?php echo Upload_Design?>" /></td>
      </tr>
    </table>
	</fieldset>
	<fieldset>
...
//设定选择sm图和原图
$image_sm = tep_db_output($row['creative_file_name']);
if(tep_not_null($row['creative_file_name_sm'])){
	$image_sm = tep_db_output($row['creative_file_name_sm']);
}
?>
    <td align="left" valign="top">
	<img src="zoom.gif" alt="<?php echo nl2br(tep_db_output($row['creative_name'])); ?>" name="Layer_dis_pic_<?php echo $row['creative_id'] ?>" class="Layer_dis_pic" id="Layer_dis_pic_<?php echo $row['creative_id'] ?>" onclick="change_pic_src('Layer_dis_pic_<?php echo $row['creative_id'] ?>','<?php echo 'creative/'.tep_db_output($row['creative_file_name']);?>' ,'Layer_load_pic_<?php echo $row['creative_id'] ?>')" />
	
	<div id="Layer_load_pic_<?php echo $row['creative_id'] ?>" class="Layer_load_pic_none"></div>
	
	<div style="width:152px; height:114px; border: 2px solid #CCCCCC;">
	<a href="<?php echo 'creative/'.tep_db_output($row['creative_file_name']);?>" target="_blank" onmouseover="auto_load_creative_for_relevant('<?php echo 'creative/'.tep_db_output($row['creative_file_name']);?>')">
	<img src="<?php echo 'creative/'.$image_sm;?>" alt="<?php echo tep_db_output($row['creative_name']); ?>" border="0" align="absmiddle" <?php echo getimgHW3hw('creative/'.$image_sm,SM_WIDTH,SM_HEIGHT);?> />	</a>	</div>
	
	<div>
	<?php
	//取得子级图片
	$creative_a_sql = tep_db_query('SELECT * FROM `creative_a` WHERE creative_id ="'.(int)$row['creative_id'].'" ');
	$J=1;
	while($creative_a_rows = tep_db_fetch_array($creative_a_sql)){
	?>
	<a href="<?php echo 'creative/'.tep_db_output($creative_a_rows['creative_a_file_name']);?>" target="_blank"><img name="" src="<?php echo 'creative/'.$creative_a_rows['creative_a_file_name_sm'] ?>" width="32" height="32" border="0" alt="<?php echo $creative_a_rows['creative_a_date'].'由'.$creative_a_rows['creative_a_upload_people'].'上传！'  ?>" /></a>
	<?php
		if($J>0 && $J%4==0){echo "</div><div>";}
		$J++;
	}
	?>
...
if($row['reviews_total']){
	$sql_reviews = tep_db_query("select * from `reviews` where  reviews_type='creative' AND reviews_type_id='".$row['creative_id']."' order by reviews_id DESC limit 3; ");
...
?>	</td>
    <td width="150" align="left" valign="top"><strong><?php echo Name?>：</strong><?php echo tep_db_output($row['creative_name']); ?><br />
      <strong><?php echo Description?>：</strong><?php echo nl2br(tep_db_output($row['creative_description'])); ?><br />
      <strong><?php echo Person?>：</strong><?php echo tep_db_output($row['creative_upload_people']); ?><br />
      <strong><?php echo DATE?>：</strong><?php echo preg_replace('/ .*$/','',$row['creative_date']); ?><br />
      <strong><?php echo Category?>：</strong><?php echo tep_db_output(get_image_group_name($row['group_id'])); ?><br />
	  
	  
	  
	  <div id="Layer_look_reviews_<?php echo $row['creative_id'] ?>" class="Layer_none">
		</div>
	  
	  
	  
	  <div id="Layer_reviews_<?php echo $row['creative_id'] ?>" class="Layer_reviews_none">
	  	<form style="margin-bottom:0px; margin-left:0px; margin-right:0px; margin-top:0px;" action="" method="get" name="FormWriteReviews<?php echo $row['creative_id'] ?>" onsubmit="submit_reviews('FormWriteReviews<?php echo $row['creative_id'] ?>', 'Layer_reviews_<?php echo $row['creative_id'] ?>'); return false;" id="FormWriteReviews<?php echo $row['creative_id'] ?>">
		<fieldset>
		<legend align="left"><img onclick="move_action('Layer_reviews_<?php echo $row['creative_id'] ?>')" style="margin-right: 5px;" src="move.gif" alt="移动" width="25" height="21" />给 <?php echo tep_db_output($row['creative_name']); ?> 评论</legend>
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
		  ...
		  <tr>
		    <td align="right">&nbsp;</td>
		    <td align="left"><input name="Submit_<?php echo $row['creative_id'] ?>" type="submit" id="Submit_<?php echo $row['creative_id'] ?>" value="<?php echo SUBMIT?>" />
		      <input name="Submit4" type="button" onclick="document.getElementById('Layer_reviews_<?php echo $row['creative_id'] ?>').className='Layer_reviews_none'" value="<?php echo CLOSE?>" />
		      <input name="reviews_type_id" type="hidden" id="reviews_type_id" value="<?php echo tep_db_output($row['creative_id']); ?>" />
		      <input name="reviews_type" type="hidden" id="reviews_type" value="creative" /></td>
		    </tr>
		  <tr>
		    <td align="right">&nbsp;</td>
		    <td align="left">&nbsp;</td>
		...
		<div id="re_msn_<?php echo $row['creative_id']?>" style="display:<?php if(!$row['reviews_total']){?>none<?php }?>"><strong><?php echo COMMENT?>：</strong>
		<?php printf(HOW_HAVE_COMMENT,'re_total_'.$row['creative_id'], (int)$row['reviews_total'])?>
		<input onclick="JavaScript:LookReviews('Layer_look_reviews_<?php echo $row['creative_id'] ?>','<?php echo $row['creative_id'] ?>')" class="link_buttom" name="" type="button" value="<?php echo VIEW_COMMENT?>" /></div>
		
	    <input name="Submit4" type="button" class="link_buttom" onclick="document.getElementById('Layer_reviews_<?php echo $row['creative_id'] ?>').className='Layer_reviews'" value="<?php echo SEND_COMMENT?>" />		</td>
