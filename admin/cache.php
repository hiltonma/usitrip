...
// check if the cache directory exists
  if (is_dir($dir_cache)) {
    if (!is_writeable($dir_cache)) $messageStack->add(ERROR_CACHE_DIRECTORY_NOT_WRITEABLE, 'error');
  } else {
...
<?php
  if ($messageStack->size < 1) {
    ...
    for ($i=0, $n=sizeof($cache_blocks); $i<$n; $i++) {
      ...
      if (file_exists($dir_cache . $cached_file)) {
        $cache_mtime = strftime(DATE_TIME_FORMAT, filemtime($dir_cache . $cached_file));
      } else {
        ...
        while ($cache_file = $dir->read()) {
          ...
          if (ereg('^' . $cached_file, $cache_file)) {
            $cache_mtime = strftime(DATE_TIME_FORMAT, filemtime($dir_cache . $cache_file));
            break;
          }
        }
