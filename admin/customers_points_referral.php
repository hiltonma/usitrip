...
array('id' => '4', 'text' => TEXT_DEFAULT_FEEDBACK_APPROVAL),
array('id' => '5', 'text' => TEXT_DEFAULT_REVIEWS_PHOTOS),
array('id' => '6', 'text' => TEXT_DEFAULT_ANSWER)
);
...
switch ($potype) {
	case '1':
		$potype_query = "where p.points_type != 'SP'";
		$heading_title = HEADING_TITLE;
		break;
	case '2':
		$potype_query = "where p.points_type = 'RF'";
		$heading_title = HEADING_TITLE_REFERRAL;
		break;
	case '3':
		$potype_query = "where p.points_type = 'RV'";
		$heading_title = HEADING_TITLE_REVIEW;
		break;
	case '4':
		$potype_query = "where p.points_type = 'FA'";
		$heading_title = HEADING_TITLE_FEEDBACK;
		break;
	case '5':
		$potype_query = "where p.points_type = 'PH'";
		$heading_title = HEADING_TITLE_TRAVELER_PHOTO;
		break;
	case '6':
		$potype_query = "where p.points_type = 'QA'";
		$heading_title = HEADING_TITLE_QUE_ANS;
		break;
	default:
		$potype_query = "where p.points_type != 'SP'";
		$heading_title = HEADING_TITLE;
...
<tr>
<td class="smallText" align="right"><?php echo TABLE_HEADING_POINTS_TYPE . ': ' .tep_draw_pull_down_menu('potype', $potype_array, '', 'onChange="this.form.submit();"') . '&nbsp;'. TABLE_HEADING_POINTS_STATUS . ':'. tep_draw_pull_down_menu('filter', $filter_array, '', 'onChange="this.form.submit();"'); ?></td>
</form></tr>
</table></td>
</tr>
</table></td>
...
$pending_points_query_raw = "select p.*, c.customers_gender, c.customers_lastname, c.customers_firstname, c.customers_email_address, c.customers_shopping_points, c.customers_points_expires from " . TABLE_CUSTOMERS_POINTS_PENDING . " p left join " . TABLE_CUSTOMERS . " c on c.customers_id = p.customer_id ".$potype_query." and p.points_type != 'RG' ".$filter." ".$search." order by ".$sort."";
...
while ($pending_points = tep_db_fetch_array($pending_points_query)) {
	...
	if ($pending_points['points_type'] == 'RV') {
		...
		$uInfo_array = $pending_points;
		$link = '<a href="' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $pending_points['orders_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_REVIEWS_EDIT) . '</a>&nbsp;';
	}
	if ($pending_points['points_type'] == 'PH') {
		...
		$uInfo_array = array_merge($pending_points, $reviews);*/
		$uInfo_array = $pending_points;
		$link = '<a href="' . tep_href_link(FILENAME_TRAVELER_PHOTOS, 'page=' . $_GET['page'] . '&tpID=' . $pending_points['orders_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_REVIEWS_EDIT) . '</a>&nbsp;';
	}

	if ($pending_points['points_type'] == 'FA') {
		$uInfo_array = $pending_points;
		$link = '<a href="'.$pending_points['feedback_other_site_url'].'" target="_blank">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_REVIEWS_EDIT) . '</a>&nbsp;';
		//' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $reviews['reviews_id'] . '&action=edit') . '
	}
	...
	if ($pending_points['points_type'] == 'QA') {
		...
		$link = '<a href="' . tep_href_link(FILENAME_QUESTION_ANSWERS, 'page=' . $_GET['page'] . '&que_id=' . $pending_points['orders_id'] . '&action=view_question') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_REVIEWS_EDIT) . '</a>&nbsp;';
		?>
		<?php
	}
	...
	$type = ($pending_points['points_type'] == 'RG') ? TEXT_WELCOME_POINTS_COMMENT : TEXT_TYPE_REVIEW;
	$type = ($pending_points['points_type'] == 'FA') ? TEXT_DEFAULT_FEEDBACK_APPROVAL : TEXT_TYPE_REVIEW;
	$type = ($pending_points['points_type'] == 'QA') ? TEXT_DEFAULT_ANSWER : TEXT_TYPE_REVIEW;

	if($pending_points['points_type'] == 'PH'){
		$type = TEXT_DEFAULT_REVIEWS_PHOTOS;
	}
	if($pending_points['points_type'] == 'FA'){
		$type = TEXT_DEFAULT_FEEDBACK_APPROVAL.'<br><a href="'.$pending_points['feedback_other_site_url'].'" target="_blank">'.$pending_points['feedback_other_site_url'].'</a>';
	}
	if($pending_points['points_type'] == 'RF'){
		$type = TEXT_TYPE_REFERRAL;
	}
...
<tr>
<td class="smallText" valign="top"><?php echo $pending_points_split->display_count($pending_points_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_ADMIN, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_ORDERS); ?></td>
<td class="smallText" align="right"><?php echo $pending_points_split->display_links($pending_points_query_numrows, MAX_DISPLAY_SEARCH_RESULTS_ADMIN, MAX_DISPLAY_PAGE_LINKS, $_GET['page'], tep_get_all_get_params(array('page', 'uID', 'action'))); ?></td>
</tr>
...
switch ($action) {
	...
	default:
		if (isset($uInfo) && is_object($uInfo)) {
			...
			if($uInfo->points_comment == 'TEXT_DEFAULT_REVIEWS_PHOTOS') {
				$uInfo->points_comment = TEXT_DEFAULT_REVIEWS_PHOTOS;
			}
			if($uInfo->points_comment == 'TEXT_DEFAULT_FEEDBACK_APPROVAL') {
				$uInfo->points_comment = TEXT_DEFAULT_FEEDBACK_APPROVAL;
			}
			if($uInfo->points_comment == 'TEXT_WELCOME_POINTS_COMMENT') {
				$uInfo->points_comment = TEXT_WELCOME_POINTS_COMMENT;
			}
