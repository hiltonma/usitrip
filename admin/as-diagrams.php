...
<?
  ...
  $Ystep = floor($maxval/4); // ¡õ¡õ 1 ¡õ¡õëþ Ñú ãÖ?Y
  ...
  { // compute optimal width for one bar (¡õ¡õ¡õ?¡õ¡õ¡õ¡õ¡õ?¡õ¡õ¡õ ß¿¡õÞè¡õ)
  ...
  { // ¡õÑã¡õ¡õ¡õ?¡õ¡õ¡õ ¡õ¡õ¡õ ß¿¡õÞè¡õ, ¡õï¹ ¡õ ¡õ¡õ¡õ?¡õ¡õ?
...
//echo "<table border=0><tr><td align=right>¡õ¡õ-?/td></tr>\n";
  if($this->showdigits)
...
//   $pc = $bar[$iy]; // todo - $iy % count($bar);
      ...
      for($ix=0; $ix<count($legend_x); $ix++)
      {
		...
		//amit added to fixed $ vl
		 if( ($lgd == 'Revenue' || $lgd == 'Avg Revenue' )&& $value > 0){	
	  ...
	  if( ($lgd == 'Revenue' || $lgd == 'Avg Revenue' )&& $smm > 0){	
        echo "   <td align=right nowrap>&nbsp;$".$smm."&nbsp;</td>\n";
		}else{
