...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('easypopulate');
...
/*comment by scs 'v_products_image_sm_1',
...
//added cpath
	...
	if ($download == 'stream'){
		...
		echo  '<a href="easypopulate.php">' . EASY_FILE_RETURN . '</a><br>';
...
//echo $login_id;
include DIR_FS_CLASSES . 'Remark.class.php';
$listrs = new Remark('easypopulate');
...
//	check files name for EPA
    if ( (strstr($localfile, 'EPA')) or ( (strstr($usrfl_name, 'EPA')) && $split==0) )  {
          ...
          echo EASY_ERROR_6 .  '<a href="' . tep_href_link(FILENAME_EASYPOPULATE) . '">' . EASY_ERROR_6a . '</a><br>';
	 die();
      }
...
//	check files name for EPA
	...
	echo EASY_LABEL_FILE_COUNT_1A . $filecount . EASY_LABEL_FILE_COUNT_2;
...
?>
      ...
      <table width="75%" border="1">
		...
		?>
		  ...
		  </select><b><?php echo EASY_LABEL_SORT;?></b><br>

     <br><b><?php echo 'Limit number of products to Download';?></b><br><br>
		...
		?><b><?php echo EASY_LABEL_LIMIT_CAT;?></b><br>
		...
		?><b><?php echo EASY_LABEL_LIMIT_MAN;?></b><br>
        ...
        <b><?php echo EASY_LABEL_PRODUCT_RANGE;?></b><br>
        <?php // below are the queries to do the counts
...
function print_el1( $item6 ) {
	echo sprintf("| %'.4s ", substr(strip_tags($item6), 0, 80));
};
		...
		for ( $categorylevel=$max_categories+1; $categorylevel>0; $categorylevel-- ){
			...
			if ( $thiscategoryname != ''){
				...
				if ( $row != '' ){
					...
					if(USE_MCACHE) MCache::update_categories(array('method'=>'insert','categories_id'=>$max_category_id)); //更新缓存-vincent-2011-4-22
				}
...
//if delete
	if ( $v_action == 'delete' ){
		...
		echo EASY_LABEL_DELETE_STATUS_1 . $v_products_id . EASY_LABEL_DELETE_STATUS_2 . '<br>';
		// Get the ID
...
MCache::update_product($delete_id);//MCache update
