...
$product_row = mysql_query($product_query);
$product_result = mysql_fetch_array($product_row);
...
$tour_agency_opr_currency = tep_get_tour_agency_operate_currency($add_product_products_id);
										...
										if($operator_result['spe_single'] > 0 || $operator_result['spe_kids'] > 0){											
											...
											if($daycount == 8)
											{
												...
												for($adate=2;$adate<=365; $adate++)
												{
													...
													$checkselected = '';
													if(isset($_POST['availabletourdate'][$add_product_products_id]) &&  $_POST['availabletourdate'][$add_product_products_id] == $formval.'::'.$prifix[$countprice].'##'.$extracharges[$countprice].'!!!'.$add_spl_products_start_day_id[$countprice]){
									...
									while($available_result = tep_db_fetch_array($available_query))
									{
											//echo "in";
												...
												if($isvaliddatecheck == "valid" && $from1 >= date('Y-m-d')){
														...
														$sel_date_irreg_array = explode('::',$_POST['availabletourdate'][$add_product_products_id]);
														if(isset($_POST['availabletourdate'][$add_product_products_id]) &&  $sel_date_irreg_array[0] == $formval){
															$checkselected = ' selected ';
														}
				...
				*/
				...
				if($is_start_date_required==true && tep_check_product_is_hotel((int)$_POST['add_product_products_id'])!=1){
					echo '<tr>
						  <td align="left" class="dataTableContent">Date:</td>
						  <td align="left" class="dataTableContent"><select name="availabletourdate['.$add_product_products_id.']" style="width:250;">
							  '.$avaliabledate.'
							</select></td>
						</tr>
						';
					 ...
					 /* Hotel Extensions - Early/Late check-in/out - start */
						...
						//}
						if($get_hotel_attr==2){
							$checkin_date_field_name = "late_hotel_checkin_date[".$add_product_products_id."]";
							$checkout_date_field_name = "late_hotel_checkout_date[".$add_product_products_id."]";							
							echo tep_draw_hidden_field('h_l_id['.HOTEL_EXT_ATTRIBUTE_OPTION_ID.']', 2); 
							echo tep_draw_hidden_field('staying_late_hotels['.$add_product_products_id.']', (int)$_POST['add_product_products_id']); 
						}else{
							$checkin_date_field_name = "early_hotel_checkin_date[".$add_product_products_id."]";
							$checkout_date_field_name = "early_hotel_checkout_date[".$add_product_products_id."]";
							echo tep_draw_hidden_field('h_e_id['.HOTEL_EXT_ATTRIBUTE_OPTION_ID.']', $get_hotel_attr); 
							echo tep_draw_hidden_field('early_arrival_hotels['.$add_product_products_id.']', (int)$_POST['add_product_products_id']);
						}
						echo tep_draw_hidden_field('is_hotel['.$add_product_products_id.']', 1);
					 ?>
					 <div class="eabo_co">
					 ...
					 <div class="eabo_co_rt">
						...
						<br />
						<input type="text" name="<?php echo $checkin_date_field_name; ?>" value="<?php echo $_POST[$checkin_date_field_name]; ?>" />&nbsp;(MM/DD/YYYY)
					 </div>
					 </div>
					 <div class="eabo_co">
					 <img src="<?php echo DIR_WS_TEMPLATE_IMAGES;?>ye<?php echo $image_ye_cntr++;?>.gif" class="fleft" alt="" />
					 <div class="eabo_co_rt">
						...
						<br />
						<input type="text" name="<?php echo $checkout_date_field_name; ?>" value="<?php echo $_POST[$checkout_date_field_name]; ?>" />&nbsp;(MM/DD/YYYY)
...
if($ama != '')
{
	foreach($ama as $key => $val)
	{
		...
		if(stripslashes($_POST['departurelocation'][$add_product_products_id]) == $departure_result['departure_time'].'::::'.$departure_result['departure_address'].', '.$departure_result['departure_full_address'])
		...
		$depart_option .= '<option value="'.$departure_result['departure_time'].'::::'.$departure_result['departure_address'].', '.$departure_result['departure_full_address'].'">'.$departure_result['departure_time'].' '.$departure_result['departure_address'].', '.$departure_result['departure_full_address'].'</option>';
				 ...
				 if($depart_option != '')
				   {
   						echo'<tr>
					  <td align="left" class="dataTableContent">Departure:</td>
					  <td align="left" class="dataTableContent"><select name="departurelocation['.$add_product_products_id.']" style="width:400px;">'.$depart_option.'</select></td>
					</tr>';
				  }
				...
				{ 
					  ...
					  echo'<tr>
						  ...
						  <td align="left" class="dataTableContent" nowrap="nowrap"><DIV id=hot-search-params-'.$add_product_products_id.' class="fleft"></DIV></td>
						</tr>';						
					  }else if($display_room_combo==0)
					  {
					  echo'<tr>
						  <td align="left" class="dataTableContent" valign="top">Tickets:</td>
						  <td align="left" class="dataTableContent"><DIV id=hot-search-params-'.$add_product_products_id.'></DIV></td>
						</tr>';
					  }
			   }
			   else
			...
			//echo '<tr><td colspan="2" align="right">'.$final_product_price.'</td></tr>';	  		
	echo '</table>';  
...
<!-- NOTE: DO NOT MODIFY THIS JAVASCRIPT BLOCK -->
      <SCRIPT language=javascript>
				<!--
					...
					{
					?>
				    refresh(<?php echo $add_product_products_id; ?>);
					<?php 
					}
					?>
				    ...
				    function setNumRooms(x, add_product_products_id) {
				        numRooms = x;
				        for (i = 0; i < x; i++) {
				        ...
				        refresh(add_product_products_id);
				    }
				    ...
				    function renderRoomSelect() {
				        var x = '';
				        x += '<select name="numberOfRooms[<?php echo $add_product_products_id; ?>]" id="numberOfRooms[<?php echo $add_product_products_id; ?>]" onchange="setNumRooms(this.options[this.selectedIndex].value, <?php echo $add_product_products_id; ?>);">';
				        for (var i = 1; i < 21; i++) {
				    ...
				    function refresh(add_product_products_id) {
				        maxChildren = 0;
				        ...
				        if (numRooms > 22) {
				            ...
				            for (var i = 0; i < numRooms; i++) {
				                ...
				                x += '<td'+cellStyle+'>';
				                x += buildSelect('room-' + i + '-adult-total['+add_product_products_id+']', 'setNumAdults(' + i + ', this.options[this.selectedIndex].value)', 1, 20, adultsPerRoom[i]);
				                x += '</td><td'+cellStyle+'>';
				                x += buildSelect('room-' + i + '-child-total['+add_product_products_id+']', '', 0, 10, childrenPerRoom[i]);//setNumChildren(' + i + ', this.options[this.selectedIndex].value)
				                x += '</td></tr>\n';
				            }
				            x += '</table>\n';
				            ...
				            for (var i = 0; i < numRooms; i++) {
				                if (childrenPerRoom[i] > 0) {
				                    ...
				                    for (var j = 0; j < childrenPerRoom[i]; j++) {
				                        ...
				                        x += '<select name="room-'+i+'-child-'+j+'-age['+add_product_products_id+']" onchange="setChildAge('+i+', '+j+', this.options[this.selectedIndex].value);">';
						...
						//alert("hot-search-params-"+add_product_products_id+"");

				        document.getElementById("hot-search-params-"+add_product_products_id+"").innerHTML = x;
				    }
