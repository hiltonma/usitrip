...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('stats_order_item_without_amount');
	...
	foreach ($_POST['prev_orderitemid'] as $orderitemid=>$status){
		 ...
		 else
		 {
			...
			tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array_locations, 'update', " orders_products_id = '".$orderitemid."' ");					
				...
				$messageStack->add_session('Selected orders has been checked on Payment Adjusted Report.', 'success');	
				
				tep_redirect(tep_href_link(FILENAME_STATS_ORDER_ITEM_WITHOUT_AMOUNT, tep_get_all_get_params(array('action','oID','products_id')).'action=view_report'));
...
//echo $login_id;
include DIR_FS_CLASSES . 'Remark.class.php';
$listrs = new Remark('stats_order_item_without_amount');
...
<table border="0" width='100%' cellspacing="0" cellpadding="2">
						<tr>
							...
							<td colspan="9" valign="top">
								<?php echo tep_draw_form('search', FILENAME_STATS_ORDER_ITEM_WITHOUT_AMOUNT, '', 'get'); ?>
...
//var order_date_purchased_end = new ctlSpiffyCalendarBox("order_date_purchased_end", "search", "order_date_purchased_to","btnDate4","<?php echo tep_get_date_disp($_GET['order_date_purchased_to']); ?>",scBTNMODE_CUSTOMBLUE);
						...
						<?php echo tep_draw_form('orderitemcheck', FILENAME_STATS_ORDER_ITEM_WITHOUT_AMOUNT, tep_get_all_get_params(array('selected_box','action')).'action=proccess_payment_cofirm', 'post'); ?>
				...
				/////   end - search coding    ///////
							...
							$items_sold_cnt =0;
						/*	
						 $prods_query_sql = "select p.agency_id, p.provider_tour_code, o.orders_id, o.orders_status, o.date_purchased, op.orders_products_id, op.payment_paid, op.products_id, op.products_model, op.products_name, op.final_price, op.products_quantity, op.final_price_cost,op.products_departure_date, op.orders_products_id, oph.cost, oph.retail, oph.last_updated_date, oph.invoice_number, oph.invoice_amount, oph.updated_by, oph.comment  from   ".TABLE_ORDERS_PRODUCTS." as op left join ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." as oph on oph.orders_products_id = op.orders_products_id, orders as o, products as p, tour_travel_agency ta where
							    ord_prod_history_id IN (SELECT max(ord_prod_history_id) FROM ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY."  GROUP BY orders_products_id)
								 ".$where." and (o.orders_status ='100002' or o.orders_status ='100006' ) and (op.final_price_cost = '0.00' or op.final_price_cost = '' or o.order_cost = '0.00' or o.order_cost='') and op.orders_id = o.orders_id and p.agency_id=ta.agency_id and  op.products_id=p.products_id ".$sortorder." ";
								 */
							
							$prods_query_sql = "select p.agency_id, p.provider_tour_code, o.orders_id, o.orders_status, o.date_purchased, op.orders_products_id, op.payment_paid, op.products_id, op.products_model, op.payment_adjusted_blocked, op.products_name, op.final_price, op.products_quantity, op.final_price_cost,op.products_departure_date, op.orders_products_id, oph.cost, oph.retail, oph.last_updated_date, oph.invoice_number, oph.invoice_amount, oph.updated_by, oph.comment from  orders as o, products as p, tour_travel_agency ta, ".TABLE_ORDERS_PRODUCTS." as op,".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." oph LEFT JOIN ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." oph2 ON oph.orders_products_id = oph2.orders_products_id
AND oph.ord_prod_history_id < oph2.ord_prod_history_id and oph.last_updated_date < oph2.last_updated_date where oph2.last_updated_date IS NULL and oph.orders_products_id = op.orders_products_id and (o.orders_status ='100002' or o.orders_status ='100006' ) and (op.final_price_cost = '0.00' or op.final_price_cost = '' or o.order_cost = '0.00' or o.order_cost='') ".$where." and op.orders_id = o.orders_id and p.agency_id=ta.agency_id and  op.products_id=p.products_id ".$sortorder." ";
