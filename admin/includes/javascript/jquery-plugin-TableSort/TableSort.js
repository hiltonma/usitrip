...
//       3、调用扩展方法，如：jQuery("#tableId").sorttable();
//
// 参数：JSON格式
//       ==普通参数==
    ...
    sorttable: function (setting) {
        ...
        headCells.each(function () {
            if (configer.ascImgUrl != "" && configer.descImgUrl != "") {
                jQuery("&nbsp;<img class='asc' src='" + configer.ascImgUrl + "' style='width:" + configer.ascImgSize + "; height:" + configer.ascImgSize + ";display:none;' title='升序' alt='升序'/>").appendTo(jQuery(this));
                jQuery("&nbsp;<img class='desc' src='" + configer.descImgUrl + "' style='width:" + configer.descImgSize + "; height:" + configer.descImgSize + ";display:none;' title='降序' alt='降序'/>").appendTo(jQuery(this));
            }
            else {
                jQuery("&nbsp;<span class='asc' style='width:12px; height:12px;display:none;' title='升序'>&#118;</span>").appendTo(jQuery(this));
                jQuery("&nbsp;<span class='desc' style='width:12px; height:12px;display:none;' title='降序'>&#94;</span>").appendTo(jQuery(this));
            }
        ...
        headCells.click(function () {
            ...
            if (lock == false) {
                ...
                if (configer.allowMask) {
                    ...
                    // 添加遮罩层
                    var mark = jQuery("<div></div>").attr("id", "TableSort_Mark").css("background-color", configer.maskColor).css("position", "absolute").css("top", tTop + "px").css("left", tLeft + "px").css("opacity", configer.maskOpacity).css("z-index", "9999").css("width", tw + "px").css("height", th + "px");
        ...
        function SetColumnOrder(headCell) {
            ...
            rs.sort(function (r1, r2) {
                var a = jQuery.trim(jQuery(r1).children("td").eq(index).text());
                var b = jQuery.trim(jQuery(r2).children("td").eq(index).text());
                if (a == b) {
                    return 0;
                }
        ...
        function IsTime(dateString) {
            ...
            var reg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})jQuery/;
            var r = dateString.match(reg);
            if (r == null) {
                var reg = /^(\d{1,4})(-|\/)(\d{1,2})\2(\d{1,2})jQuery/;
                var r = dateString.match(reg);
            }
