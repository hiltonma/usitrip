function TinyMCE_Engine() {
	this.charset = "gb2312";	/*默认为gb2312版本，有big5和gb2312可选*/
	this.majorVersion = "2";
...
TinyMCE_Engine.prototype = {
	init: function(settings) {
		...
		this.uniqueURL = 'http://tinymce.moxiecode.cp/mce_temp_url';
		...
		} else this.settings['editor_css'] = tinyMCE.baseURL + "/themes/" + theme + "/css/editor_ui.css";
		...
		if (this.configs.length == 0) {
			if (this.isSafari && this.getParam('safari_warning', false)) alert("Safari support is very limited and should be considered experimental.\nSo there is no need to even submit bugreports on this early version.\nYou can disable this message by setting: safari_warning option to false");
			if (typeof(TinyMCECompressed) == "undefined") {
				tinyMCE.addEvent(window, "DOMContentLoaded", TinyMCE_Engine.prototype.onLoad);
				if (tinyMCE.isMSIE && !tinyMCE.isOpera) {
					if (document.body) tinyMCE.addEvent(document.body, "readystatechange", TinyMCE_Engine.prototype.onLoad);
	...
	_addUnloadEvents: function() {
		if (tinyMCE.isMSIE) {
			...
			if (tinyMCE.settings['add_unload_trigger']) tinyMCE.addEvent(window, "unload",
			function() {
	...
	updateContent: function(form_element_name) {
		...
		for (var n in tinyMCE.instances) {
			...
			if (inst.formElement == formElement) {
				...
				if (!tinyMCE.isMSIE) doc.body.innerHTML = tinyMCE._cleanupHTML(inst, doc, this.settings, doc.body, inst.visualAid);
			}
		}
	},
	...
	_createIFrame: function(replace_element, doc, win) {
		...
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) iframe.setAttribute("src", this.settings['default_document']);
		iframe.style.width = aw;
		iframe.style.height = ah;
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) replace_element.outerHTML = iframe.outerHTML;
		else replace_element.parentNode.replaceChild(iframe, replace_element);
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) return win.frames[id];
		else return iframe;
	},
	...
	removeTinyMCEFormElements: function(form_obj) {
		if (typeof(form_obj) == "undefined" || form_obj == null) return;
		if (form_obj.nodeName != "FORM") {
			if (form_obj.form) form_obj = form_obj.form;
			else form_obj = tinyMCE.getParentElement(form_obj, "form");
		}
		if (form_obj == null) return;
		for (var i = 0; i < form_obj.elements.length; i++) {
			var elementId = form_obj.elements[i].name ? form_obj.elements[i].name: form_obj.elements[i].id;
			if (elementId.indexOf('mce_editor_') == 0) form_obj.elements[i].disabled = true;
		}
	},
	handleEvent: function(e) {
		...
		case "keydown":
			...
			if (tinyMCE.isMSIE && e.type == "keydown" && e.keyCode == 13) tinyMCE.enterKeyElement = tinyMCE.selectedInstance.getFocusElement();
			...
			if (tinyMCE.isMSIE && e.ctrlKey) window.setTimeout('tinyMCE.triggerNodeChange(false);', 1);
			break;
		case "mousedown":
		case "mouseup":
		case "click":
		...
		for (var c = 0; c < tinyMCE.configs.length; c++) {
			...
			case "exact":
				...
				for (var i = 0; i < elements.length; i++) {
					...
					if (tinyMCE.getAttrib(element, "class").indexOf(deselector) != -1) continue;
			...
			case "textareas":
				...
				for (var i = 0; i < nodeList.length; i++) {
					...
					if (selector != '' && tinyMCE.getAttrib(elm, "class").indexOf(selector) == -1) continue;
					if (selector != '') trigger = selector != "" ? "true": "";
					if (tinyMCE.getAttrib(elm, "class").indexOf(deselector) != -1) continue;
			...
			for (var i = 0; i < elementRefAr.length; i++) {
				...
				if (tinyMCE.settings['ask'] || tinyMCE.settings['convert_on_click']) {
					if (tinyMCE.isGecko) {
						...
						tinyMCE.addEvent(element, "focus",
						function(e) {
							window.setTimeout(function() {
						...
						if (element.nodeName != "TEXTAREA" && element.nodeName != "INPUT") tinyMCE.addEvent(element, "click",
						function(e) {
							window.setTimeout(function() {
								TinyMCE_Engine.prototype.confirmAdd(e, settings);
							},
						...
						tinyMCE.addEvent(element, "click",
						function() {
							TinyMCE_Engine.prototype.confirmAdd(null, settings);
						});
					}
	...
	getLang: function(name, default_value, parse_entities, va) {
		var v = (typeof(tinyMCELang[name]) == "undefined") ? default_value: tinyMCELang[name],
		n;
	...
	importPluginLanguagePack: function(name, valid_languages) {
		var lang = "en",
		b = tinyMCE.baseURL + '/plugins/' + name;
	...
	applyTemplate: function(h, as) {
		...
		if (ar && ar.length > 0) {
			for (i = ar.length - 1; i >= 0; i--) {
				...
				if (s.indexOf('lang_') == 0 && tinyMCELang[s]) h = tinyMCE.replaceVar(h, s, tinyMCELang[s]);
				else if (as && as[s]) h = tinyMCE.replaceVar(h, s, as[s]);
	...
	handleVisualAid: function(el, deep, state, inst) {
		...
		if (deep && el.hasChildNodes()) {
			for (var i = 0; i < el.childNodes.length; i++) tinyMCE.handleVisualAid(el.childNodes[i], deep, state, inst);
		}
	},
					...
					for (var i = 0; i < csses.length; i++) {
						...
						if (selectorText) {
							...
							for (var c = 0; c < rules.length; c++) {
								if (rules[c].indexOf(' ') != -1 || rules[c].indexOf(':') != -1 || rules[c].indexOf('mceItem') != -1) continue;
								if (rules[c] == "." + tinyMCE.settings['visual_table_class'] || rules[c].indexOf('mceEditable') != -1 || rules[c].indexOf('mceNonEditable') != -1) continue;
...
TinyMCE_Control.prototype = {
	...
	repaint: function() {
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) return;
		try {
	...
	autoResetDesignMode: function() {
		if (!tinyMCE.isMSIE && this.isHidden() && tinyMCE.getParam('auto_reset_designmode')) eval('try { this.getDoc().designMode = "On"; } catch(e) {}');
	},
	isHidden: function() {
	...
	execCommand: function(command, user_interface, value) {
		...
		if (focusElm && focusElm.nodeName == "IMG") {
			...
			switch (command) {
			case "JustifyLeft":
				if (align == 'left') img.removeAttribute('align');
			...
			case "JustifyCenter":
				...
				return;
			case "JustifyRight":
				if (align == 'right') img.removeAttribute('align');
		...
		case "InsertOrderedList":
			...
			break;
		case "Strikethrough":
			if (tinyMCE.isSafari) this.execCommand("mceInsertContent", false, "<strike>" + this.selection.getSelectedHTML() + "</strike>");
			else this.getDoc().execCommand(command, user_interface, value);
			tinyMCE.triggerNodeChange();
			break;
		case "mceSelectNode":
		...
		case "FormatBlock":
			if (value == null || value == "") {
				...
				if (value == '<div>' && tinyMCE.isGecko) value = 'div';
	...
	_onAdd: function(replace_element, form_element_name, target_document) {
		...
		if (replace_element.nodeName == "TEXTAREA" || replace_element.nodeName == "INPUT") this.startContent = replace_element.value;
		...
		if (replace_element.nodeName != "TEXTAREA" && replace_element.nodeName != "INPUT") {
			...
			html += '</span>';
			if (tinyMCE.isGecko) html = hc + html;
			else html += hc;
			if (tinyMCE.isGecko) {
		...
		if (!tinyMCE.isMSIE) {
			...
			if (tElm && tElm.nodeName == "SPAN") tElm = tinyMCE._createIFrame(tElm, targetDoc, targetDoc.parentWindow);
	...
	setBaseHREF: function(u) {
		...
		if (!b) {
			...
			if (u == "" || u == null) b.parentNode.removeChild(b);
			else b.setAttribute('href', u);
		}
	},
	...
	triggerSave: function(skip_cleanup, skip_callback) {
		...
		htm = tinyMCE.regexpReplace(htm, "&#34;", "&quot;", "gi");
		htm = tinyMCE.regexpReplace(htm, "&#94;", "^", "gi");
		if(tinyMCE.charset=="gb2312"){
			htm = simplized(htm);	/*转换成简体文字*/
		}
		if(tinyMCE.charset=="big5"){
			htm = traditionalized(htm);	/*转换成繁体文字*/
		}
...
TinyMCE_Engine.prototype.cleanupHTMLCode = function(s) {
	...
	if (tinyMCE.isMSIE) s = s.replace(/<p><hr \/><\/p>/gi, "<hr>");
...
TinyMCE_Engine.prototype.convertFontsToSpans = function(doc) {
	...
	if (fsClasses != '') fsClasses = fsClasses.replace(/\s+/, '').split(',');
	else fsClasses = null;
	...
	for (var i = 0; i < s.length; i++) {
		...
		if (fSize != "") {
			...
			if (fSize > 0 && fSize < 8) {
				if (fsClasses != null) tinyMCE.setAttrib(s[i], 'class', fsClasses[fSize - 1]);
				else s[i].style.fontSize = sizes[fSize - 1];
			}
...
TinyMCE_Engine.prototype._cleanupHTML = function(inst, doc, config, elm, visual, on_save, on_submit) {
	...
	c.sourceIndex = -1;
	if (s.cleanup_serializer == "xml") h = c.serializeNodeAsXML(elm);
	else h = c.serializeNodeAsHTML(elm);
...
TinyMCE_Cleanup.prototype = {
	init: function(s) {
		...
		if (s.invalid_elements != '') this.iveRe = this._arrayToRe(s.invalid_elements.toUpperCase().split(','), 'g', '^(', ')$');
	...
	parseRuleStr: function(s) {
		...
		for (x = 0; x < ta.length; x++) {
			...
			p = this.split(/\[|\]/, s);
			if (p == null || p.length < 1) t = s.toUpperCase();
			else t = p[0].toUpperCase();
			tn = this.split('/', t);
	...
	serializeNodeAsHTML: function(n) {
		...
		case 1:
			hc = n.hasChildNodes();
			if ((tinyMCE.isMSIE && !tinyMCE.isOpera) && n.nodeName.indexOf('/') != -1) break;
			if (this.vElementsRe.test(n.nodeName) && (!this.iveRe || !this.iveRe.test(n.nodeName))) {
				...
				if (r.vAttribsReIsWild) {
					...
					for (i = at.length - 1; i > -1; i--) {
						no = at[i];
						if (no.specified && r.vAttribsRe.test(no.nodeName)) h += this._serializeAttribute(n, r, no.nodeName);
					}
				} else {
					for (i = r.vAttribs.length - 1; i > -1; i--) h += this._serializeAttribute(n, r, r.vAttribs[i]);
				}
				if (!this.settings.on_save) {
					at = this.mceAttribs;
					for (no in at) {
						if (at[no]) h += this._serializeAttribute(n, r, at[no]);
					}
				}
				if (this.closeElementsRe.test(n.nodeName)) return h + ' />';
				h += '>';
				if (this.isMSIE && this.codeElementsRe.test(n.nodeName)) h += n.innerHTML;
			}
			break;
		case 3:
			if (n.parentNode && this.codeElementsRe.test(n.parentNode.nodeName)) return this.isMSIE ? '': n.nodeValue;
	...
	xmlEncode: function(s) {
		...
		case "numeric":
			for (i = 0, l = s.length; i < l; i++) {
				c = s.charCodeAt(i);
				if (c > 127 || c == 60 || c == 62 || c == 38 || c == 39 || c == 34) o += '&#' + c + ";";
				else o += String.fromCharCode(c);
			}
			return o;
		}
	...
	_getAttrib: function(e, n, d) {
		...
		if (this.isMSIE && n == "http-equiv") v = e.httpEquiv;
...
TinyMCE_Engine.prototype.getElementsByAttributeValue = function(n, e, a, v) {
	var i, nl = n.getElementsByTagName(e),
	o = new Array();
	for (i = 0; i < nl.length; i++) {
...
TinyMCE_Engine.prototype.getParentBlockElement = function(n) {
	while (n) {
		if (this.isBlockElement(n)) return n;
		n = n.parentNode;
	}
	return null;
};
...
TinyMCE_Engine.prototype.getOuterHTML = function(e) {
	if (tinyMCE.isMSIE) return e.outerHTML;
...
TinyMCE_Engine.prototype.getNodeTree = function(n, na, t, nn) {
	var i;
	if (typeof(t) == "undefined" || n.nodeType == t && (typeof(nn) == "undefined" || n.nodeName == nn)) na[na.length] = n;
	if (n.hasChildNodes()) {
		for (i = 0; i < n.childNodes.length; i++) tinyMCE.getNodeTree(n.childNodes[i], na, t, nn);
	}
	return na;
};
...
TinyMCE_Engine.prototype.convertAbsoluteURLToRelativeURL = function(base_url, url_to_relative) {
	...
	for (var i = breakPoint - 1; i < strTok2.length; i++) {
		if (i != (breakPoint - 1)) outPath += "/" + strTok2[i];
		else outPath += strTok2[i];
	}
	...
	if (targetURL.path == "" && !targetURL.anchor) targetURL.path = "/";
...
TinyMCE_Engine.prototype.convertRelativeToAbsoluteURL = function(base_url, relative_url) {
	...
	if (relative_url == "" || relative_url.charAt(0) == '/' || relative_url.indexOf('://') != -1 || relative_url.indexOf('mailto:') != -1 || relative_url.indexOf('javascript:') != -1) return relative_url;
...
TinyMCE_Engine.prototype._setEventsEnabled = function(node, state) {
	var events = new Array('onfocus', 'onblur', 'onclick', 'ondblclick', 'onmousedown', 'onmouseup', 'onmouseover', 'onmousemove', 'onmouseout', 'onkeypress', 'onkeydown', 'onkeydown', 'onkeyup');
		...
		for (var i = 0; i < elms.length; i++) {
			...
			for (var x = 0; x < events.length; x++) {
				if ((event = tinyMCE.getAttrib(elms[i], events[x])) != '') {
					...
					if (!state) event = "return true;" + event;
					else event = event.replace(/^return true;/gi, '');
...
TinyMCE_Selection.prototype = {
	getSelectedHTML: function() {
		...
		if (tinyMCE.isGecko) e.appendChild(r.cloneContents());
		else e.innerHTML = r.htmlText;
	...
	getBookmark: function(simple) {
		...
		if (tinyMCE.isSafari || tinyMCE.isGecko) return {
			rng: rng,
			scrollX: sx,
			scrollY: sy
		};
	...
	moveToBookmark: function(bookmark) {
		...
		if (tinyMCE.isMSIE) {
			...
			if (bookmark.tag) {
				...
				if (nl.length > bookmark.index) rng.addElement(nl[bookmark.index]);
			} else {
		...
		if (tinyMCE.isGecko) {
			...
			if (nl.length > bookmark.start) rng.setStart(nl[bookmark.start].childNodes[bookmark.startIndex], bookmark.startOffset);
			nl = doc.getElementsByTagName(bookmark.endTag);
			if (nl.length > bookmark.end) rng.setEnd(nl[bookmark.end].childNodes[bookmark.endIndex], bookmark.endOffset);
			sel.removeAllRanges();
			sel.addRange(rng);
			win.scrollTo(bookmark.scrollX, bookmark.scrollY);
			return true;
	...
	getSel: function() {
		var inst = this.instance;
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) return inst.getDoc().selection;
	...
	getRng: function() {
		...
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) return sel.createRange();
		if (tinyMCE.isSafari && !sel.getRangeAt) return '' + window.getSelection();
	...
	getFocusElement: function() {
		...
		if (tinyMCE.isMSIE && !tinyMCE.isOpera) {
			...
			if (inst.isHidden()) return inst.getBody();
			var sel = this.getSel();
			var rng = this.getRng();
...
var TinyMCE_ForceParagraphs = {
	_insertPara: function(inst, e) {
		...
		if (tinyMCE.getParentElement(startBlock, "OL,UL") != null) return false;
	...
	_handleBackSpace: function(inst) {
		...
		if (sn && sn.nextSibling && sn.nextSibling.nodeName == "BR") sn.nextSibling.parentNode.removeChild(sn.nextSibling);
		return false;
	}
};
