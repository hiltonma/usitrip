...
$PHPTHUMB_CONFIG['document_root'] = DIR_FS_CATALOG;//realpath((getenv('DOCUMENT_ROOT') && ereg('^'.preg_quote(realpath(getenv('DOCUMENT_ROOT'))), realpath(__FILE__))) ? getenv('DOCUMENT_ROOT') : str_replace(dirname(@$_SERVER['PHP_SELF']), '', str_replace(DIRECTORY_SEPARATOR, '/', dirname(__FILE__))));

// * Cache directory configuration (choose only one of these - leave the other lines commented-out):
$PHPTHUMB_CONFIG['cache_directory'] = DIR_FS_CATALOG.'images/eticket_files/thumbs/'; // set the cache directory to an absolute directory for all source images
