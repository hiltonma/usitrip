<?php
/*
  $Id: affiliate_banners_build.php,v 2.00 2003/10/12

  OSC-Affiliate

  Contribution based on:

  osCommerce, Open Source E-Commerce Solutions
  http://www.oscommerce.com

  Copyright (c) 2002 - 2003 osCommerce

  Released under the GNU General Public License
*/

define('NAVBAR_TITLE', 'Affiliate Banner');
define('HEADING_TITLE', 'Affiliate Program - Affiliate Links');

define('TEXT_AFFILIATE_NAME', 'Banner Name:');
define('TEXT_INFORMATION', 'Obtain a range of affiliate links that you can display on your website.');
define('TEXT_AFFILIATE_INFO', 'Copy the code shown below and paste into your website');
define('TEXT_AFFILIATE_CATEGORIES_INDIVIDUAL_BANNER','BUILD-A-CATEGORY-LINK');
define('TEXT_AFFILIATE_CATEGORIES_INDIVIDUAL_BANNER_INFO','Enter the <b>category number</b> you wish to link to and press the enter&nbsp;');
define('TEXT_AFFILIATE_VALIDPRODUCTS', 'Click Here:');
define('TEXT_AFFILIATE_CATEGORIES_INDIVIDUAL_BANNER_VIEW', 'to view available categories/products.');
define('TEXT_AFFILIATE_CATEGORIES_INDIVIDUAL_BANNER_HELP', 'Select the category number from the popup window and enter the number in the Build Category Link field.');

define('TEXT_VALID_CATEGORIES_LIST', 'Available Categories List');
define('TEXT_VALID_CATEGORIES_ID', 'Categories #');
define('TEXT_VALID_CATEGORIES_NAME', 'Categories Name');

define('TEXT_CLOSE_WINDOW', '<u>Close Window</u> [x]');

define('IMAGE_BUTTON_BUILD_A_LINK', 'BUILD-A-LINK');

//amti added for products
define('TEXT_AFFILIATE_INDIVIDUAL_BANNER','BUILD-A-PRODUCT-LINK');
define('TEXT_AFFILIATE_INDIVIDUAL_BANNER_INFO','Enter the <b>product number</b> you wish to link to and press the enter&nbsp;');
define('TEXT_AFFILIATE_INDIVIDUAL_BANNER_VIEW', 'to view available categories/products.');
define('TEXT_AFFILIATE_INDIVIDUAL_BANNER_HELP', 'Select the product number from the popup window and enter the number in the Build A Link field.');

define('TEXT_VALID_PRODUCTS_LIST', 'Available Products List');
define('TEXT_VALID_PRODUCTS_ID', 'Product #');
define('TEXT_VALID_PRODUCTS_NAME', 'Products Name');

?>