 ...
 * 将某个时区的时间转成系统时区对应的时间
 * @author Howard by 2012-10-20
 * @param Y-m-d H:i:s $zone_date_time 时间字符串
 * @param string $zone 时区名称如：Asia/Shanghai、America/Los_Angeles、Pacific/Honolulu[夏威夷]、America/New_York，具体请参考php手册"所支持的时区列表"部分
 * @return string, format[Y-m-d H:i:s]
 */
function zone_time_to_system_zone_time($zone_date_time, $zone){
	$sys_time = time();
	$now_zone_date = strtotime(get_zone_time($zone));	
	$final_zone_date = strtotime($zone_date_time);
	$gap_date = $final_zone_date - $now_zone_date;
	$system_zone_time = date('Y-m-d H:i:s',($sys_time + $gap_date));
	return $system_zone_time;
}
/**
 * 将系统时区的某个时间转成目标时区对应的时间
 * @author Howard by 2012-10-20
 * @param Y-m-d H:i:s $sys_zone_date_time 时间字符串
 * @param string $target_zone 时区名称如：Asia/Shanghai、America/Los_Angeles、Pacific/Honolulu[夏威夷]、America/New_York，具体请参考php手册"所支持的时区列表"部分
 * @return string, format[Y-m-d H:i:s]
 */
function system_zone_time_to_zone_time($sys_zone_date_time, $target_zone){
	$sys_time = time();
	$target_now_zone_time = strtotime(get_zone_time($target_zone));
	$gap_date = strtotime($sys_zone_date_time) - $sys_time;
	$target_zone_time = date('Y-m-d H:i:s',($target_now_zone_time + $gap_date));
	return $target_zone_time;
}
