...
*/
  ...
  function read_faq ($language) {
	...
	function browse_faq ($language,$HTTP_GET_VARS) {
	 ...
	 $qery_extra = " and ip2c.categories_id='".$HTTP_GET_VARS['cID']."' ";
	 }
	  ...
	  if ($HTTP_GET_VARS['faq_lang'] != '') {
	 	 ...
	 	 $query_lang = " WHERE 1=1 ";
	  }
	  
	  $query = "SELECT *, DATE_FORMAT(date, '%d.%m.%y') AS d FROM " . TABLE_FAQ . ", " . TABLE_FAQ_TO_CATEGORIES . " ip2c $query_lang and faq.faq_id=ip2c.faq_id ".$qery_extra." ORDER BY v_order";
	  ...
	  while ($buffer = tep_db_fetch_array($daftar)) {
			$categories_query = tep_db_query("select icd.categories_id, icd.categories_name from " . TABLE_FAQ_TO_CATEGORIES . " ip2c left join " . TABLE_FAQ_CATEGORIES_DESCRIPTION . " icd on icd.categories_id = ip2c.categories_id   where ip2c.faq_id = '" . (int)$buffer['faq_id'] . "' ".$qery_extra." and icd.language_id = '" . (int)$languages_id . "'");
			$category = tep_db_fetch_array($categories_query);
