...
class sales_report {
	var $mode, $globalStartDate, $startDate, $endDate, $actDate, $showDate, $showDateEnd, $sortString, $status, $outlet, $payStatus;

	function sales_report($mode, $startDate = 0, $endDate = 0, $sort = 0, $statusFilter = 0, $filter = 0, $payStatus = 'all' ) {
		...
		//amit commented $this->queryOrderCnt = "SELECT count(o.orders_id) as order_cnt FROM " . TABLE_ORDERS . " o";
		$this->queryOrderCnt = "SELECT count(o.orders_id) as order_cnt FROM " . TABLE_ORDERS . " as o , " . TABLE_ORDERS_TOTAL . " as ot ";


		// queries for item details count

		if(isset($_GET['group_country']) && $_GET['group_country'] == '1' ){
			//amit added to count later
			$this->queryOrderExternal = "SELECT count(o.orders_id) as order_cnt FROM " . TABLE_ORDERS . " as o , " . TABLE_ORDERS_TOTAL . " as ot ";


			$this->queryItemCnt = "SELECT count(o.orders_id) as order_group_cnt,o.orders_id,o.customers_country, op.products_id as pid, op.orders_products_id, op.products_name as pname, sum(op.products_quantity) as pquant, sum(op.final_price * op.products_quantity) as psum, sum(op.final_price_cost * op.products_quantity) as psum_cost, op.products_tax as ptax FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op ";
		}else{
		...
		// query for shipping
		$this->queryShipping = "SELECT sum(ot.value) as shipping FROM " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot ";

		if($_GET['group_country'] == '1'){
			...
			switch ($sort) {
				case '0':
					...
					break;
				case '1':
					$this->sortString = " order by o.customers_country,pname asc ";
					break;
				case '2':
					$this->sortString = " order by o.customers_country,pname desc";
					break;
				case '3':
					$this->sortString = " order by o.customers_country,pquant asc, pname asc";
					break;
				case '4':
					$this->sortString = " order by o.customers_country,pquant desc, pname asc";
					break;
				case '5':
					$this->sortString = " order by o.customers_country,psum asc, pname asc";
					break;
				case '6':
					$this->sortString = " order by o.customers_country,psum desc, pname asc";
					break;
			}
	...
	function getNext() {
		...
		if(isset($_GET['group_country']) && $_GET['group_country'] == '1' ){
			$this->queryOrderExternal = "SELECT count(o.orders_id) as order_ext_cnt FROM " . TABLE_ORDERS . " as o , " . TABLE_ORDERS_TOTAL . " as ot ";
		}
		...
		if ($this->payStatus !='all') {
			$tmpS = ", " . TABLE_ORDERS_PRODUCTS . " op ";
			if(strpos(str_replace(' ','',$this->queryOrderCnt), str_replace(' ','',$tmpS)) === false){
				$this->queryOrderCnt .= $tmpS;
			}
			if(strpos(str_replace(' ','',$this->queryShipping), str_replace(' ','',$tmpS)) === false){
				$this->queryShipping .= $tmpS;
			}
			if(strpos(str_replace(' ','',$this->queryItemCnt), str_replace(' ','',$tmpS)) === false){
				$this->queryItemCnt .= $tmpS;
			}
			if(strpos(str_replace(' ','',$this->queryOrderExternal), str_replace(' ','',$tmpS)) === false){
				$this->queryOrderExternal .= $tmpS;
			}
			
			$filterString .= " AND op.orders_products_payment_status = '" . $this->payStatus . "' ";
			$filterString .= " AND op.orders_id = o.orders_id ";
		}
		
		if(isset($_GET['group_country']) && $_GET['group_country'] == '1' ){
			$this->queryOrderExternal .= " WHERE o.orders_id = ot.orders_id AND ot.class = 'ot_total' AND o.date_purchased >= '" . tep_db_input(date("Y-m-d\TH:i:s", $sd)) . "' AND o.date_purchased < '" . tep_db_input(date("Y-m-d\TH:i:s", $ed)) . "'" . $filterString . "";
			$_SESSION['externalquery'] = $this->queryOrderExternal;
		}

		$rqOrders = tep_db_query($this->queryOrderCnt . " WHERE o.orders_id = ot.orders_id and ot.class = 'ot_total' and o.date_purchased >= '" . tep_db_input(date("Y-m-d\TH:i:s", $sd)) . "' AND o.date_purchased < '" . tep_db_input(date("Y-m-d\TH:i:s", $ed)) . "'" . $filterString);
		$order = tep_db_fetch_array($rqOrders);

		$rqShipping = tep_db_query($this->queryShipping . " WHERE ot.orders_id = o.orders_id AND  ot.class = 'ot_shipping' AND o.date_purchased >= '" . tep_db_input(date("Y-m-d\TH:i:s", $sd)) . "' AND o.date_purchased < '" . tep_db_input(date("Y-m-d\TH:i:s", $ed)) . "'" . $filterString);
		$shipping = tep_db_fetch_array($rqShipping);


		if(isset($_GET['group_country']) && $_GET['group_country'] == '1' ){
			if($_GET['detail']==0){
				$rqItems = tep_db_query($this->queryItemCnt . " WHERE o.orders_id = op.orders_id AND o.date_purchased >= '" . tep_db_input(date("Y-m-d\TH:i:s", $sd)) . "' AND o.date_purchased < '" . tep_db_input(date("Y-m-d\TH:i:s", $ed)) . "'" . $filterString . " group by o.customers_country " . $this->sortString);
			}else{
				$rqItems = tep_db_query($this->queryItemCnt . " WHERE o.orders_id = op.orders_id AND o.date_purchased >= '" . tep_db_input(date("Y-m-d\TH:i:s", $sd)) . "' AND o.date_purchased < '" . tep_db_input(date("Y-m-d\TH:i:s", $ed)) . "'" . $filterString . " group by o.customers_country,pid " . $this->sortString );
			}

		}else{
			$rqItems = tep_db_query($this->queryItemCnt . " WHERE o.orders_id = op.orders_id AND o.date_purchased >= '" . tep_db_input(date("Y-m-d\TH:i:s", $sd)) . "' AND o.date_purchased < '" . tep_db_input(date("Y-m-d\TH:i:s", $ed)) . "'" . $filterString . " group by pid " . $this->sortString);
		}
		...
		while ($resp[$cnt] = tep_db_fetch_array($rqItems)) {
			...
			$price = $resp[$cnt]['psum'] / max($resp[$cnt]['pquant'],1);
			$price_cost = $resp[$cnt]['psum_cost'] / max($resp[$cnt]['pquant'],1);
