...
var st = "";
st += "<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=4><TR><TD><FONT FACE='Verdana,Arial,Helvetica' SIZE=-2>";
for(var i = 0; i < this.legends.length; i++) {
if(!this.legends[i]) continue;
if(i >= this.rows.length) break;
st += "<IMG SRC=images/"+i+".gif BORDER=1 HSPACE=3>"+this.legends[i]+"</td><TD><FONT FACE='Verdana,Arial,Helvetica' SIZE=-2>";
}
st += "</FONT></TD></TR></TABLE>";
return st;
}
...
str += "<FONT FACE='Verdana,Arial,Helvetica' SIZE=-1>";
str += g.title;
str += "</FONT></TH></TR>\n";
}
if(g.yLabel) {
...
str += "<FONT FACE='Verdana,Arial,Helvetica' SIZE=-1>"+g.yLabel+"</FONT></TD>\n";
}
if(g.posMax > 0) {
if(!g.yLabel) str += "<TR>\n";
...
str += "<IMG SRC=\"images/clear.gif\" WIDTH="+parseInt(g.hscale)+" HEIGHT=5>";
str += "</TD>\n<TD VALIGN=BOTTOM";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += ">";
}
...
str += "<IMG SRC=\"images/clear.gif\" ALIGN=TOP BORDER=0 WIDTH="+parseInt(g.hscale)+
" HEIGHT=5>";
str += "</TD>\n<TD VALIGN=TOP";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
str += ">";
}
str += "<IMG SRC=\"images/clear.gif\" ALIGN=TOP WIDTH=1 BORDER=0 HEIGHT=5>";
}
str += "</TD>\n";
...
if(g.scale) str += "<TD COLSPAN=3></TD>\n";
if(g.yLabel) str += "<TD></TD>\n";
str += "<TH COLSPAN="+g.c+" HEIGHT=25 VALIGN=BOTTOM><FONT FACE='Verdana,Arial,Helvetica' SIZE=-1>";
str += g.xLabel;
str += "</FONT></TH></TR>\n";
}
str += "</TABLE>\n";
...
st += "><FONT FACE='Verdana,Arial,Helvetica' SIZE=-3><I>";
st += g.dates[i*g.skip] || "";
st += "</I></FONT></TD>\n";
}
...
var H = h - 3;
for(var i = 0; i < n; i++) {
st += "<FONT FACE=Verdana,Arial,Helvetica SIZE=-3><I>"+(g.scale*-1*(i+1)+g.offset)+"</I></FONT>";
...
var H = h - 3;
for(var i = 0; i < n; i++) {
st += "<FONT FACE=Verdana,Arial,Helvetica SIZE=-3><I>"+(g.scale*(n-1)-g.scale*i+g.offset);
if(g.relative) st += "%";
st += "</I></FONT>";
st += "<IMG SRC=images/clear.gif WIDTH=1 HEIGHT="+H+"><BR>\n";
}
...
str += "<FONT FACE='Verdana,Arial,Helvetica' SIZE=-1>"+g.yLabel+"</FONT></TH>\n";
}
if(g.posMax > 0) {
if(!g.yLabel) str += "<TR>\n";
if(g.scale) str += _writeScaleGraph(g, 0, g.posMax);
...
if(!drawn) str += "<IMG SRC=images/clear.gif WIDTH="+parseInt(g.hscale)+" HEIGHT=1>";
str += "</TD>\n";
str += "<TD";
if(g.bgColor) str += " BGCOLOR=\""+g.bgColor+"\"";
...
if(g.legends && g.posMax != 0) {
str += "<TR>\n";
str += "<TD colspan=99 align=center>";
str += g.writeLegend();
str += "</TD></tr>\n";
}
