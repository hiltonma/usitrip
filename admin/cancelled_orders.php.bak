...
if($_GET['ajax']=="true"){
	...
	$remark = new Remark('cancelled_orders');
		 ...
		 else
		 {
		   ...
		   /*$sql_data_array_locations = array(		   
			...
			tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_data_array_locations, 'update', " orders_products_id = '".$orderitemid."' ");					  */
				...
				tep_redirect(tep_href_link(FILENAME_CANCELLED_ORDERS, tep_get_all_get_params(array('action','oID','products_id')).'action=view_report'));
...
<script type="text/javascript" src="includes/jquery-1.3.2/jquery-1.3.2.min.js"></script>
	...
	<body marginwidth="0" marginheight="0" topmargin="0" bottommargin="0" leftmargin="0" rightmargin="0" bgcolor="#FFFFFF">
		<? if ( ! $_REQUEST['print'] ) {
			...
			//echo $login_id;
			include DIR_FS_CLASSES . 'Remark.class.php';
			$listrs = new Remark('cancelled_orders');
...
<table border="0" width='100%' cellspacing="0" cellpadding="2">
						<tr>
							...
							<td colspan="9" valign="top">
								<?php echo tep_draw_form('search', FILENAME_CANCELLED_ORDERS, '', 'get'); ?>
...
var order_cancelled_time_start = new ctlSpiffyCalendarBox("order_cancelled_time_start", "search", "order_cancelled_time_from","btnDate3","<?php echo tep_get_date_disp($_GET['order_cancelled_time_from']); ?>",scBTNMODE_CUSTOMBLUE);
var order_cancelled_time_end = new ctlSpiffyCalendarBox("order_cancelled_time_end", "search", "order_cancelled_time_to","btnDate4","<?php echo tep_get_date_disp($_GET['order_cancelled_time_to']); ?>",scBTNMODE_CUSTOMBLUE);*/
...
//--></script>
								<table width="99%"  cellpadding="2" cellspacing="0" border="0">
									...
									<tr>
										<td class="smallText" colspan="2">Order Cancelled Date From:
										<?php echo tep_draw_input_field('order_cancelled_time_from', tep_get_date_disp($_GET['order_cancelled_time_from']), ' class="textTime" style="ime-mode: disabled;" onclick="GeCalendar.SetUnlimited(1);GeCalendar.OutPutType=\'m/d/y\'; GeCalendar.SetDate(this);"');?>
										<!--script type="text/javascript">order_cancelled_time_start.writeControl(); order_cancelled_time_start.dateFormat="MM/dd/yyyy";</script--> To:
										<?php echo tep_draw_input_field('order_cancelled_time_to', tep_get_date_disp($_GET['order_cancelled_time_to']), ' class="textTime" style="ime-mode: disabled;" onclick="GeCalendar.SetUnlimited(1);GeCalendar.OutPutType=\'m/d/y\'; GeCalendar.SetDate(this);"');?>
											<!--script type="text/javascript">order_cancelled_time_end.writeControl(); order_cancelled_time_end.dateFormat="MM/dd/yyyy";</script-->
										</td>
									</tr>
						...
						<?php echo tep_draw_form('orderitemcheck', FILENAME_CANCELLED_ORDERS, tep_get_all_get_params(array('selected_box','action')).'action=proccess_payment_cofirm', 'post','onSubmit="return confirm(\'Are you sure you want to check selected orders for this report?\');"'); ?>
			
						<tr bgcolor="#E35762">
							<td class="main" colspan="17"><?php echo RED_ROW_EXPLAINATION_TEXT; ?></td>
						</tr>
						<tr>
							<td class="dataTableContent" colspan="10">&nbsp;</td>
						</tr>
						<tr class="dataTableHeadingRow">
						<td class="dataTableHeadingContent">Order Cancelled Date</td>
				...
				/////   start - search coding    ///////
						$where = ' ';
						 ...
						 if(isset($_GET['paid_filter']) && ($_GET['paid_filter'] == '0' || $_GET['paid_filter'] == '1'))
						  ...
						  $make_end_date = tep_get_date_db($HTTP_GET_VARS['dept_end_date'])." 23:59:59";;
						 ...
						 $where .= " and  o.date_purchased <= '" . $make_end_date . "' ";
						  ...
						  if ((isset($HTTP_GET_VARS['order_last_modified_from']) && tep_not_null($HTTP_GET_VARS['order_last_modified_from'])) && (isset($HTTP_GET_VARS['order_last_modified_to']) && tep_not_null($HTTP_GET_VARS['order_last_modified_to'])) ) {
							  ...
							  $make_end_date = tep_get_date_db($HTTP_GET_VARS['order_last_modified_to']). " 23:59:59";
						 ...
						 if ((isset($HTTP_GET_VARS['order_cancelled_time_from']) && tep_not_null($HTTP_GET_VARS['order_cancelled_time_from'])) && (isset($HTTP_GET_VARS['order_cancelled_time_to']) && tep_not_null($HTTP_GET_VARS['order_cancelled_time_to'])) ) {
						  
							  $make_start_date = tep_get_date_db($HTTP_GET_VARS['order_cancelled_time_from']). " 00:00:00";					 
							  $make_end_date = tep_get_date_db($HTTP_GET_VARS['order_cancelled_time_to']). " 23:59:59";
							  $where .= " and  o.cancelled_time >= '" . $make_start_date . "' and o.cancelled_time <= '" . $make_end_date . "' ";
						  
						  }else if ((isset($HTTP_GET_VARS['order_cancelled_time_from']) && tep_not_null($HTTP_GET_VARS['order_cancelled_time_from'])) && (!isset($HTTP_GET_VARS['order_cancelled_time_to']) or !tep_not_null($HTTP_GET_VARS['order_cancelled_time_to'])) ) {
							
							  $make_start_date = tep_get_date_db($HTTP_GET_VARS['order_cancelled_time_from']) . " 00:00:00";
							  $where .= " and  o.cancelled_time >= '" . $make_start_date . "' ";
						  
						  }else if ((!isset($HTTP_GET_VARS['order_cancelled_time_from']) or !tep_not_null($HTTP_GET_VARS['order_cancelled_time_from'])) && (isset($HTTP_GET_VARS['order_cancelled_time_to']) && tep_not_null($HTTP_GET_VARS['order_cancelled_time_to'])) ) {
							
							 $make_end_date = tep_get_date_db($HTTP_GET_VARS['order_cancelled_time_to']) . " 23:59:59";
							 $where .= " and  o.cancelled_time <= '" . $make_end_date . "' ";
				...
				/////   end - search coding    ///////
					  ...
					  $prods_query_sql = "select o.last_modified, p.agency_id, p.provider_tour_code, o.orders_id, o.orders_status,o.cancelled_time, o.date_purchased, op.orders_products_id, op.payment_paid, op.products_id, op.products_model,op.payment_adjusted_blocked, op.products_name, op.final_price, op.products_quantity, op.final_price_cost,op.products_departure_date, op.orders_products_id, oph.cost, oph.retail, oph.last_updated_date, oph.invoice_number, oph.invoice_amount, oph.updated_by, oph.comment  from   ".TABLE_ORDERS_PRODUCTS." as op, ".TABLE_ORDERS_PRODUCTS_UPDATE_HISTORY." as oph, orders as o, products as p where oph.orders_products_id = op.orders_products_id
								 ".$where." and o.orders_status=6  and op.orders_id = o.orders_id and  op.products_id=p.products_id group by oph.orders_products_id order by o.last_modified desc";
							...
							//  $product_split = new splitPageResults($HTTP_GET_VARS['page'], MAX_DISPLAY_SEARCH_RESULTS_ADMIN, $prods_query_sql, $prods_query_numrows);
							 ...
							 while ($prods = tep_db_fetch_array($prods_query)) {							
									...
									<tr <?php if($prods['payment_paid'] == '1'){echo  ' bgcolor="#E35762"'; }else{ echo 'class="dataTableRow"'; } ?>>
									<td class="dataTableContent"><?= tep_date_short($prods['cancelled_time']); ?></td>
									...
									<input class="buttonstyle" name="orderitemid[]" type="checkbox" value="<?php echo $prods['orders_products_id'];?>" <?php if($prods['payment_adjusted_blocked']==1) { echo ' checked="checked"'; } ?> ></td>
						  ...
						  <tr class="dataTableHeadingRow">
						  <td colspan="5"></td>			
						  <td class="dataTableHeadingContent" colspan="4" align="center">
