...
//        $gv_order_amount += 0.001;
    ...
    function credit_selection() {
      ...
      if (tep_db_num_rows($gv_query)) {
        ...
        $image_submit = '<input type="image" name="submit_redeem" onClick="submitFunction()" src="' . DIR_WS_TEMPLATE_IMAGES.'buttons/' . $language . '/button_redeem.gif" border="0" alt="' . IMAGE_REDEEM_VOUCHER . '" title = "' . IMAGE_REDEEM_VOUCHER . '">';
    ...
    function collect_posts() {
      global $currencies, $HTTP_POST_VARS, $customer_id, $coupon_no, $REMOTE_ADDR;
      if ($HTTP_POST_VARS['gv_redeem_code']) {
        $gv_query = tep_db_query("select coupon_id, coupon_type, coupon_amount from " . TABLE_COUPONS . " where coupon_code = '" . $HTTP_POST_VARS['gv_redeem_code'] . "'");
        $gv_result = tep_db_fetch_array($gv_query);
