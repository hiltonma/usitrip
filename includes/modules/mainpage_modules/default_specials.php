...
<!-- default_specials //-->
<?php
 $new2 = tep_db_query("select specials_id from " . TABLE_SPECIALS . " where status = '1'");
//    $specialsm_query = tep_db_query($new2);
  
  // Don't display if there is no downloadable product
    if (tep_db_num_rows($new2) > 0) {
?>
  <tr>
      </tr>
      <tr>
...
$info_box_contents = array();
 ...
 $new1 = tep_db_query("select p.products_id, pd.products_name, p.products_price, p.products_tax_class_id, p.products_image, s.specials_new_products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_SPECIALS . " s where p.products_status = '1' and s.products_id = p.products_id and p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and s.status = '1' order by s.specials_date_added, rand() DESC limit " . MAX_DISPLAY_SPECIAL_PRODUCTS);

 $info_box_contents = array();
  $row = 0;
  $col = 0;
  while ($default_specials = tep_db_fetch_array($new1)) {
