...
require(DIR_FS_LANGUAGES . $language . '/' . FILENAME_FAQDESK_REVIEWS_ARTICLE);
...
and faqdesk_id = '" . $HTTP_GET_VARS['faqdesk_id'] . "'
");
...
. TABLE_FAQDESK_REVIEWS . " where approved='1' and faqdesk_id = '" . $HTTP_GET_VARS['faqdesk_id'] . "' order by reviews_id DESC
");
...
<?php
echo '<a href="' . tep_href_link(FILENAME_FAQDESK_REVIEWS_WRITE, $get_params, 'NONSSL') . '">' . tep_image_button('button_submit_reviews.gif', IMAGE_BUTTON_WRITE_REVIEW) . '</a>';
?>
		</td>
	</tr>
</table>
