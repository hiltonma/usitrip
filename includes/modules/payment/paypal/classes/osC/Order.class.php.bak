...
class PayPal_osC_Order {
  ...
  function loadTransactionSessionInfo($txn_sign) {
    ...
    $orders_session_query = tep_db_query("select orders_id, content_type, payment_title, language, language_id, billto, sendto, currency, currency_value, payment_amount, payment_currency from " . TABLE_ORDERS_SESSION_INFO . " where txn_signature ='" . tep_db_input($txn_signature) . "' limit 1");
  ...
  function loadOrdersSessionInfo() {
    $orders_session_query = tep_db_query("select content_type, payment_title, language, language_id, billto, sendto, currency, currency_value from " . TABLE_ORDERS_SESSION_INFO . " where orders_id ='" . (int)$this->orderID . "' limit 1");
    if(tep_db_num_rows($orders_session_query)) {
      $orders_session = tep_db_fetch_array($orders_session_query);
      $this->setCommonVars($orders_session);
    }
  ...
  function updateProducts(&$order,&$currencies) {
    ...
    // Stock Update - Joao Correia
      ...
      $products_ordered_price = $this->displayPrice($order->products[$i]['final_price'],$order->products[$i]['tax'],$order->products[$i]['qty']);
		...
		$order->products[$i]['roomattributes'][1] = $order->products[$i]['products_room_info'];
		...
		$order->products[$i]['dateattributes'][0] = $order->products[$i]['products_departure_date'];
		$order->products[$i]['dateattributes'][1] = $order->products[$i]['products_departure_time'];
		$order->products[$i]['dateattributes'][2] = $order->products[$i]['products_departure_location'];
		$display_departure_time_location = "\n- Departure Time & Location: ".$order->products[$i]['dateattributes'][0].' '.$order->products[$i]['dateattributes'][1].' '.$order->products[$i]['dateattributes'][2];
		$this->products_ordered .= $order->products[$i]['qty'] . ' x ' . $order->products[$i]['name'] . ' (' . $order->products[$i]['model'] . ') = ' . $products_ordered_price . $products_ordered_attributes . $displayattributesinmail . $display_departure_time_location . "\n";
		//code by bhavik end
  ...
  function notifyCustomer(&$order) {
	...
	$reservation_receipt_email = sprintf(EMAIL_TEXT_SUBJECT,$this->orderID,$this->orderID);
	
    tep_mail($order->customer['name'], $order->customer['email_address'], $reservation_receipt_email, $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);

    // send emails to other people
    if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
      tep_mail('', SEND_EXTRA_ORDER_EMAILS_TO, $reservation_receipt_email, $email_order, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
    }
  }
}//end class
?>
