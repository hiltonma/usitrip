...
class PayPal_TransactionDetails {
  ...
  function simpleQuery($paypal_id) {
    ...
    $ipn_query = tep_db_query("select " . implode(',',array_merge((array)$info,(array)$txn)) . " from " . tep_db_input($this->paymentTableName) . " where paypal_id = '" . (int)$paypal_id . "'");
  ...
  function query($txn_id) {
    ...
    $customer = array('first_name','last_name','payer_business_name','address_name','address_street','address_city','address_state','address_zip','address_country','address_status','payer_email','payer_id','payer_status','memo');
    $ipn_query = tep_db_query("select " . implode(',',array_merge((array)$info,(array)$txn,(array)$customer)) . " from " . tep_db_input($this->paymentTableName) . " where txn_id = '" . tep_db_input($transaction_id) . "'");
    if (tep_db_num_rows($ipn_query)) {
      $ipn = tep_db_fetch_array($ipn_query);
      $this->info = $this->getSQLDataElements($ipn,$info);
      $this->txn = $this->getSQLDataElements($ipn,$txn);
