...
//admin workaround!
$modules_directory = defined('DIR_FS_CATALOG_MODULES') ? DIR_FS_CATALOG_MODULES : DIR_FS_MODULES;
...
class PayPal_IPN extends PayPal_Client_Connector {
  ...
  function insert() {
    ...
    if($this->isAuction()) $this->processAuction($this->ipn_id);
    if($debug->enabled) $debug->add(IPN_TXN_INSERT,sprintf(IPN_TXN_INSERT_MSG,$this->ipn_id));
    return $this->ipn_id;
  }

  function updateStatus($paypal_id,$pendingReason) {
  ...
  function setSQLDataElements($varnames) {
    ...
    for($i=0; $i<$nVars; $i++) {
      if(isset($this->key[$varnames[$i]]) && tep_not_null($this->key[$varnames[$i]]))
  ...
  function processAuction($paypal_id) {
    if(defined('TABLE_PAYPAL_AUCTION')) {
      if(isset($this->key['for_auction']) && $this->key['for_auction'] === 'true' && strlen($this->key['item_number']) > 0) {
