...
class parse_email{
	 ...
	 */
	private function set_mail_content($content){
		if (is_array($content)) {
			//var_dump($content);
	...
	public function format_mail_text($email_text = '', $separator = ''){
		...
		if (empty($separator) == true) { //邮件中不分多块内容.
			...
			if (is_array($email_text)){
				...
				$text = $this->decodeText(join('',$email_text),$this->email_header['content-transfer-encoding'][0]);
				
			} elseif( is_string($email_text) && $email_text != '') {
				$text = $this->decodeText($email_text,$this->email_header['content-transfer-encoding'][0]);
			}
			if ($type == 'text/plain') {
				$text = '<pre>' . $text . '</pre>';
			}
	...
	private function replace_img(){
		if (is_array($this->email_text['attachment'])) {
...
try{
	...
	$a = new parse_email('/var/www/html/888trip.com/wwwroot/admin/email/'.$email_name);
	...
	header('Content-Type:text/html;charset=2312');
	var_dump($e);
	print_r($e);
}
?>