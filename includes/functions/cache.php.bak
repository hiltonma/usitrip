...
// Cache the categories box
  function tep_cache_coolmenu($auto_expire = false, $refresh = false) {
    ...
    if (($refresh == true) || !read_cache($cache_output, 'coolmenu.php-' . $language . '.cache' . $cPath, $auto_expire)) {
...
//get default template name
 ...
 //set all possible catagories X.php names    
     ob_start();
       include(DIR_FS_TEMPLATES . TEMPLATE_NAME . '/boxes/coolmenu.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
      write_cache($cache_output, 'coolmenu-' . $language . '.cache' . $cPath);
    }
...
//get default template name
 ...
 //set all possible catagories X.php names    
     ob_start();
       include(DIR_FS_TEMPLATES . TEMPLATE_NAME . '/boxes/categories.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
...
function tep_cache_categories_box1($auto_expire = false, $refresh = false) {
    ...
    if (($refresh == true) || !read_cache($cache_output, 'categories_box1-' . $language . '.cache' . $cPath, $auto_expire)) {
...
//get default template name
 ...
 //set all possible catagories X.php names    
     ob_start();
       include(DIR_FS_TEMPLATES . TEMPLATE_NAME . '/boxes/categories1.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
      write_cache($cache_output, 'categories_box1-' . $language . '.cache' . $cPath);
    }
...
function tep_cache_categories_box4($auto_expire = false, $refresh = false) {
    ...
    if (($refresh == true) || !read_cache($cache_output, 'categories_box4-' . $language . '.cache' . $cPath, $auto_expire)) {
...
//get default template name
 ...
 //set all possible catagories X.php names    
     ob_start();
       include(DIR_FS_TEMPLATES . TEMPLATE_NAME . '/boxes/categories4.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
      write_cache($cache_output, 'categories_box4-' . $language . '.cache' . $cPath);
    }
  ...
  $cptemplate1 = tep_db_fetch_array($customer_template_query);
   ...
   if (tep_not_null($cptemplate1['template_selected'])) {
    ...
    if (($refresh == true) || !read_cache($cache_output, 'manufacturers_box-' . $language . '.cache' . $manufacturers_id, $auto_expire)) {
     ob_start();
      include(DIR_FS_TEMPLATES . TEMPLATE_NAME . '/boxes/manufacturers.php');
      $cache_output = ob_get_contents();
      ob_end_clean();
...
// Cache the also purchased module
  function tep_cache_also_purchased($auto_expire = false, $refresh = false) {
    ...
    if (($refresh == true) || !read_cache($cache_output, 'also_purchased-' . $language . '.cache' . $HTTP_GET_VARS['products_id'], $auto_expire)) {
      ob_start();
      include(DIR_FS_MODULES . FILENAME_ALSO_PURCHASED_PRODUCTS);
      $cache_output = ob_get_contents();
      ob_end_clean();
