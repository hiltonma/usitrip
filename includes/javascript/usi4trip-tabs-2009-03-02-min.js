...
function Depature_JumpRedictUrl(selObj){window.location=selObj.options[selObj.selectedIndex].value;}
...
if(qur_array['rn']!=''&&qur_array['rn']!=null){hashString+='rn-'+qur_array['rn']+'/';}
if(qur_array['page1']!=''&&qur_array['page1']!=null){hashString+='vp-'+qur_array['page1']+'/';}
if(qur_array['tours_type']!=''&&qur_array['tours_type']!=null){hashString+='tt-'+qur_array['tours_type']+'/';}
if(qur_array['tours_type1']!=''&&qur_array['tours_type1']!=null){hashString+='vtt-'+qur_array['tours_type1']+'/';}
if(qur_array['products_durations']!=''&&qur_array['products_durations']!=null){hashString+='dn-'+qur_array['products_durations']+'/';}
if(qur_array['products_durations1']!=''&&qur_array['products_durations1']!=null){hashString+='vdn-'+qur_array['products_durations1']+'/';}
if(qur_array['departure_city_id']!=''&&qur_array['departure_city_id']!=null){hashString+='dc-'+qur_array['departure_city_id']+'/';}
if(qur_array['departure_city_id1']!=''&&qur_array['departure_city_id1']!=null){hashString+='vdc-'+qur_array['departure_city_id1']+'/';}
if(qur_array['sort2']!=''&&qur_array['sort2']!=null){hashString+='s-'+qur_array['sort2']+'/';}
if(qur_array['sort1']!=''&&qur_array['sort1']!=null){hashString+='vs-'+qur_array['sort1']+'/';}
if(qur_array['products_id']!=''&&qur_array['products_id']!=null&&qur_array['products_id']!=0){hashString+='pid-'+qur_array['products_id']+'/';}
...
clearForm('product_queston_write');try{document.getElementById("noquestion_id_div").style.display="none";}catch(e){}}else if(trim(splt[0])=="question_answer_new_added"){fadeOut('success_qa_ans_fad_out_id',20,5000);objDiv.innerHTML=splt[1];}else if(trim(splt[0])=="successtoggeldiv"){objDiv.innerHTML=splt[2];document.getElementById(trim(splt[1])).style.display="";document.getElementById("div_singin_or_without_signin").style.display="none";}else{objDiv.innerHTML=XMLHttpRequestObject.responseText;}}
catch(e){}}
else
{if(ifLoading)
{try
...
for(i=0;i<document.getElementById(idForm).elements.length-1;i++)
{strReplaceTemp=document.getElementById(idForm).elements[i].name.replace(/\[\]/i,"");if(addhistoryhash==true){if(document.getElementById(idForm).elements[i].value!=''){if(strReplaceTemp=='page'){if(hashString.search("/p-")=='-1'&&hashString.search("/#p-")=='-1'){hashString+='p-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='rn'){if(hashString.search("/rn-")=='-1'&&hashString.search("/#rn-")=='-1'){hashString+='p-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='page1'){if(hashString.search("/vp-")=='-1'&&hashString.search("/#vp-")=='-1'){hashString+='vp-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='tours_type'){if(hashString.search("/tt-")=='-1'&&hashString.search("/#tt-")=='-1'){hashString+='tt-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='tours_type1'){if(hashString.search("/vtt-")=='-1'&&hashString.search("/#vtt-")=='-1'){hashString+='vtt-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='products_durations'){if(hashString.search("/dn-")=='-1'&&hashString.search("/#dn-")=='-1'){hashString+='dn-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='products_durations1'){if(hashString.search("/vdn-")=='-1'&&hashString.search("/#vdn-")=='-1'){hashString+='vdn-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='departure_city_id'){if(hashString.search("/dc-")=='-1'&&hashString.search("/#dc-")=='-1'){hashString+='dc-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='departure_city_id1'){if(hashString.search("/vdc-")=='-1'&&hashString.search("/#vdc-")=='-1'){hashString+='vdc-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='sort'){if(hashString.search("/s-")=='-1'&&hashString.search("/#s-")=='-1'){hashString+='s-'+document.getElementById(idForm).elements[i].value+'/';}}else if(strReplaceTemp=='sort1'){if(hashString.search("/vs-")=='-1'&&hashString.search("/#vs-")=='-1'){hashString+='vs-'+document.getElementById(idForm).elements[i].value+'/';}}}}
postData+="&aryFormData["+strReplaceTemp+"][]="+document.getElementById(idForm).elements[i].value.replace(/&/g,"@@amp;");}
postData+="&parm="+new Date().getTime();if(addhistoryhash==true){if(hashString!=''){unFocus.History.addHistory(hashString);if(hashString!=''){document.getElementById('last_ajax_hash').innerHTML=hashString;}}}
try
...
throw({description:'ÆÉµÈî£ßù¡õ¡õ½²ÏéÇÚÜí'});for(var i=0;i<ls.length;i++){ls[i].index=i;if(i==index){ls[i].className='show_hovertab';c[i].className='show_dis';ls[i].parentNode.last=ls[i];}
...
function initTab_search(nid,cid,action,defaultIndex){var ls=g(nid).getElementsByTagName('li');var cc=g(cid).childNodes;var c=[];var index=defaultIndex?defaultIndex:0;for(var i=0;i<cc.length;i++)if(cc[i].nodeType==1)c.push(cc[i]);if(ls.length!=c.length)
throw({description:'²Ëµ¥ºÍÄÚÈÝÊýÁ¿²»¶ÔÓ¦'});for(var i=0;i<ls.length;i++){ls[i].index=i;if(i==index){ls[i].className='hovertabx';c[i].className='disx'
ls[i].parentNode.last=ls[i];}
addEvent(ls[i],action,function(e){var self=window.event?window.event.srcElement:e?e.target:null;if(self.parentNode.last){self.parentNode.last.className='normaltabx';c[self.parentNode.last.index].className='undisx';};self.className='hovertabx';c[self.index].className='disx';self.parentNode.last=self;});}}
...
this.addPanelEventListeners(tabs[i],panels[i]);this.showPanel(this.defaultTab);};function showDiv(){document.getElementById('popDiv').style.display='block';document.getElementById('bg').style.display='block';}
function closeDiv(){document.getElementById('popDiv').style.display='none';document.getElementById('bg').style.display='none';}
var curObj=null;function document_onclick(){if(window.event.srcElement.tagName=='A'||window.event.srcElement.tagName=='TD'){if(curObj!=null)
curObj.style.background='';curObj=window.event.srcElement;curObj.style.background='#223C6A';}}
function showdh(n){for(var i=1;i<=5;i++){document.getElementById("dh"+i).className="unsel"}
document.getElementById("dh"+n).className="sel"}
