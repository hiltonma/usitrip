...
var Span_Div = 'div';
if(typeof(validation_div_span)!="undefined"){
	Span_Div = 'span';
}
...
Object.extend(String.prototype, {
  ...
  escapeHTML: function() {
    var div = document.createElement(Span_Div);
...
if(/Konqueror|Safari|KHTML/.test(navigator.userAgent))
  ['', 'Form', 'Input', 'TextArea', 'Select'].each(function(tag) {
    ...
    klass.prototype = document.createElement(tag ? tag.toLowerCase() : Span_Div).__proto__;
  });
...
String.prototype.parseStyle = function(){
  var element = Element.extend(document.createElement(Span_Div));
  element.innerHTML = '<'+Span_Div+' style="' + this + '"></'+Span_Div+'>';
...
Validation.prototype = {
	...
	onSubmit :  function(ev){
		if(!this.validate() || submit_disabled==true){
			Event.stop(ev);
			if(submit_disabled==true){
				if(document.getElementById('submit_msn') != null){
					//alert(document.getElementById('submit_msn').innerHTML);
				}else{
					//alert('Do not duplicate submit!');
				}
			}
		}else{
			if(document.getElementById('submit_msn') != null){
				document.getElementById('submit_msn').style.display ='';
			}
			//on checkout_payment.tpl.php use the var stop_use_var_submit_disabled
			if(typeof(stop_use_var_submit_disabled) == "undefined" || stop_use_var_submit_disabled!=true){
...
Object.extend(Validation, {
	...
	test : function(name, elm, useTitle) {
		...
		if(Validation.isVisible(elm) && !v.test($F(elm), elm)) {
			if(!elm[prop]) {
				...
				if(advice == null) {
					...
					advice = '<'+Span_Div+' class="validation-advice" id="advice-' + name + '-' + Validation.getElmID(elm) +'" style="display:none;">' + errorMsg + '</'+Span_Div+'>'// padding-left:7px;
			...
			elm[prop] = true;
			if(elm.removeClassName('validation-passed')){
				var elmid = elm.id;
				if(elm.hasClassName('greenok')){					
					document.getElementById('greenok-'+elmid).innerHTML = '';
				}
				//elm.removeClassName('greenok');
			}
			...
			if(advice != null){ advice.hide();
				/*if(!document.getElementById('greenok-'+elm.id)){
					new Insertion.After(elm, greenok);//advice.innerHTML = '<span style="color: #00CC00"><b>Ok</b></span>';
				}
				*/
				//alert(After(elm));
				//advice.innerHTML = '<img src="image/right_icon.gif" />';	
			}
			elm[prop] = '';
			elm.removeClassName('validation-failed');
			if(elm.addClassName('validation-passed')){
				if(elm.id!='' && elm.id!=null && elm.id!=undefined){
				if(!elm.hasClassName('greenok')){
					elm.addClassName('greenok');
					//alert(elm.id);					
					greenok = '<span id="greenok-'+elm.id+'" class="validation_right"></span>';					
					new Insertion.After(elm, greenok);
					
				}else{
					var elmid = elm.id;
					if(document.getElementById('greenok-'+elmid).innerHTML==''){						
						document.getElementById('greenok-'+elmid).innerHTML = '<img src="image/icons/tip_right.gif" />';
					}
				}
				}
			}
		...
		if(elm.hasClassName('validation-passed')){
...
Validation.addAllThese([
	...
	['validate-email', '请输入有效的电子邮箱！', function (v) {
				return Validation.get('IsEmpty').test(v) || /^\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/.test(v)
			}],
	['validate-phone', '请输入带国家区号的有效的手机号码！如：+86 13512345678', function (v) {
				return Validation.get('IsEmpty').test(v) || /^(\+?)(\d{1,4}\s?)\d{8,11}$/.test(v)
			}],
	...
	['validate-date-us', 'Please use this date format: mm/dd/yyyy. e.g. 03/17/2006 for March 17, 2006', function(v) {
				if(Validation.get('IsEmpty').test(v)) return true;
				var regex = /^(\d{2})\/(\d{2})\/(\d{4})$/;
				if(!regex.test(v)) return false;
	...
	['validate-selection', select_option_error, function(v,elm){
				return elm.options ? elm.selectedIndex > 0 : !Validation.get('IsEmpty').test(v);
				//return !Validation.get('IsEmpty').test(v);
			}],
	['validate-selection-blank', select_option_error, function(v,elm){
				//return elm.options ? elm.selectedIndex > 0 : !Validation.get('IsEmpty').test(v);
				return !Validation.get('IsEmpty').test(v);
			}],
	['validate-one-required', radio_error, function (v,elm) {
				var p = elm.parentNode;
