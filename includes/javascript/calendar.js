...
G_calendar.prototype={
    ...
    {
        var MaxYearInterval = 100; //最大年分与最小年份的区间跨度为100年
		var SatrYearToTheYearNum = 90; //最早年份离今年的为多少年
        ...
        var n = m - SatrYearToTheYearNum;
        if (n < 1000) n = 1000;
        if (n + MaxYearInterval > 9999) n = 9974;
        ...
        for (var i = n; i < n + MaxYearInterval; i++)
        {
            if (i == m){
    ...
    DayClick:function(mm,dd,ynJQ,info,selectIndex)  //点击显示框选取日期，主输入函数*************
    {
        ...
        if (this.ClickObject){
            ...
            if(ynJQ==false){
                ...
                if(typeof(DateForMat)!="undefined" && DateForMat=="mm/dd/yyyy"){
					this.InputObject.value= mm+"/"+dd+"/"+yy;
				}else{
...
//var DateForMat = "mm/dd/yyyy";	//此值应在引入本JS文件之前定义
