...
function AddToCart(){
	...
	ajax.onreadystatechange = function() {
		...
		if (ajax.readyState == 4 && ajax.status == 200 ) {
			...
			if(!is_error)window.location = '<?php echo tep_href_link(CONTENT_SHOPPING_CART.'.php')?>';
		}
...
if((int)$rows['maximum_no_of_guest']){
	...
	//$maxPerRoomPeopleNum = 3; 后台已经有设置不用管这个
}
?>
<?php
/* 删除没有价格的房间选项 start {*/
...
$_ckeck_sql = 'SELECT products_id,products_single,products_double,products_triple,products_quadr,products_kids,is_cruises FROM '.TABLE_PRODUCTS.' WHERE products_id ="'.(int)$products_id.'" AND products_durations>1 AND products_durations_type="0" ';
$price_ckeck_sql = tep_db_query($_ckeck_sql);
...
function renderRoomSelectForTravelCompanion(){
    ...
    x += '<option value="0"><?php echo db_to_html('不选择') ?>';
    <?php
    /* 结伴同游选项 */
    if(TRAVEL_COMPANION_OFF_ON=='true'){
    ?>
    //x += '<option value="1"'+(numRooms == 1 ? ' selected' : '')+'>' + '<?php echo db_to_html('结伴同游');?>';
    x += '<option value="1">' + '<?php echo db_to_html('结伴同游');?>';
    <?php }?>
...
function SetAsGoodComment(reviews_id, good_or_bad){
	...
	function SetShowSteps3(){
		...
		for(var i=0; i<loop_num; i++){
			...
			Steps3Html +='<dd><label><?php echo db_to_html("成人： ")?></label>'+ From.elements["room-" +i+ "-adult-total"].value +'<?php echo db_to_html("人")?></dd>';		
			
			if(From.elements["room-" +i+ "-child-total"].value > 0){ 					
				Steps3Html +='<dd><label><?php echo db_to_html("小孩: ")?></label>'+ From.elements["room-" +i+ "-child-total"].value +'<?php echo db_to_html("人")?></dd>';			
			 }
			if(typeof(From.elements["room-" +i+ "-bed"])!="undefined"){
				Steps3Html +='<dd class="size"><label><?php echo db_to_html("床型: ")?></label>'+ jQuery(From.elements["room-" +i+ "-bed"]).find("option:selected").text() +'</dd>';
			}
...
<?php //清除购买盒中的所有邮轮的产品属性选项，使其为不选中状态?>
function cleanAllCruisesOptionForBookingBox(){
	...
	?>
	jQuery('input[id^="radioid_<?= $oId?>"]').attr("checked", false);
	<?php
	}
	?>
}
...
<?php //大日历价格框的代码start {?>
//var data = <?php /*此函数必须在product_info.php下面才能调用 echo tep_get_product_month_price_datas((int)$products_id) */?>;
/**
 * 日历生成类
...
function lwkCalendar(element,inputTxt,data){
	this.caption = {
		'month' : '<?php echo db_to_html("月份")?>',
		'week'  : ['<?php echo db_to_html("星期日")?>','<?php echo db_to_html("星期一")?>','<?php echo db_to_html("星期二")?>','<?php echo db_to_html("星期三")?>','<?php echo db_to_html("星期四")?>','<?php echo db_to_html("星期五")?>','<?php echo db_to_html("星期六")?>']
	};
	this.innerObj = null;
...
lwkCalendar.prototype = {
		...
		if (detail != undefined && detail != '' && detail != null){
			if (detail.constructor == Array || detail.constructor == Object) {
				for (var items in detail) {
					rtn += '<td><span style="width:' + this.liWidth + ';"><nobr>' + detail[items]['title'] + '<\/nobr><\/span><em>' + detail[items]['text'] + '<?php echo db_to_html('/人')?><\/em><\/td>';
					j++;
				}
			} else {
				rtn += '<td>' + detail + '<\/td>';
	...
	setDay : function(year,month,data){
		...
		this.yearTxt.innerHTML = yy + '<?php echo db_to_html("年")?><br/>' + mm + '<?php echo db_to_html("月")?>';
