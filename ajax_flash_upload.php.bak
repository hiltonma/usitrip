...
?>
<script type="text/javascript" src="jquery-1.3.2/jquery.flash.js"></script>
<script type="text/javascript" src="jquery-1.3.2/jquery.jqUploader.js"></script>
...
<div id="page">
 ...
 <form action="<?= tep_href_link('flash_upload.php');?>" method="post" enctype="multipart/form-data" name="<?=$form_name?>" id="<?=$form_name?>" target="<?= $iframe_id?>" onSubmit="check_upload_submit(this); return false;" class="a_form">
    ...
    <input name="FileDomain" type="hidden" />
	<input name="MAX_FILE_SIZE" value="104857600" type="hidden" />
	<ol>
      <li id="example1">

	<input name="FileDomain" type="file" id="example1_field" />
      </li>
    </ol>
	
	<input class="jb_fb_all" type="submit" id="submit_images_<?=$form_name?>" title="benginupload" value="<?= db_to_html('开始上传')?>" /><label id="LoadingIcon<?=$form_name?>"></label><br />
	<span><?= db_to_html('允许上传jpg,gif,png格式的图片！'.$width_height_px)?></span>
 </form>

 <div>
 <iframe name='<?= $iframe_id?>' id="<?= $iframe_id?>" style="display:none" ></iframe>
 </div>
 </div>
...
function get_uploaded_file(){
	if(document.getElementById("undefined")!=null){
		...
		window.setTimeout('get_uploaded_file()',200);
	}
}
jQuery(document).ready(function(){
	get_uploaded_file();
