var DD_belatedPNG = {
	...
	createVmlStyleSheet: function() {
		...
		if (c.styleSheet) {
			...
			c.addRule("img." + this.ns + "_sizeFinder", "behavior:none; border:none; position:absolute; z-index:-1; top:-10000px; visibility:hidden;");
			...
			d = d.styleSheet;
			d.addRule(this.ns + "\\:\\*", "{display: none !important;}");
			d.addRule("img." + this.ns + "_sizeFinder", "{display: none !important;}");
		}
	},
	...
	vmlFill: function(l) {
		...
		if (m.backgroundImage != "none" || l.isImg) {
			...
			if (!j.imgSize[l.vmlBg]) {
				...
				i.runtimeStyle.cssText = "behavior:none; position:absolute; left:-10000px; top:-10000px; border:none; margin:0; padding:0;";
				n = function() {
	...
	vmlOffsets: function(x) {
		...
		b = {
			W: x.clientWidth + 1,
			H: x.clientHeight + 1,
			w: this.imgSize[x.vmlBg].width,
			h: this.imgSize[x.vmlBg].height,
			L: x.offsetLeft,
			T: x.offsetTop,
			bLW: x.clientLeft,
			bTW: x.clientTop
		...
		if (x.isImg !== true || (x.isImg === true && x.isPlace === true)) {
			for (q in u) {
	...
	fixPng: function(m) {
		m.style.behavior = "none";
		var j = DD_belatedPNG,
		e, k, i, l, n = "";
		if (m.nodeName == "BODY" || m.nodeName == "TD" || m.nodeName == "TR") {
			return;
		}
		m.isImg = false;
		m.isPlace = false;
		if (m.nodeName == "IMG") {
			n = m.src.toLowerCase();
			if (n.search(/\.png$/) != -1 || n.search(new RegExp(j.placeholder + "$")) != -1) {
				m.isImg = true;
				m.style.visibility = "hidden";
				if (n.search(new RegExp(j.placeholder + "$")) != -1) {
					m.isPlace = true;
				}
			} else {
				return;
			...
			if (m.currentStyle.backgroundImage.toLowerCase().search(".png") == -1) {
				return;
			}
		}
		m.vml = {
		...
		for (i in m.vml) {
			if (m.vml.hasOwnProperty(i)) {
				for (l in e) {
					if (e.hasOwnProperty(l)) {
						k = j.ns + ":" + l;
						m.vml[i][l] = document.createElement(k);
					}
				}
				m.vml[i].shape.stroked = false;
				m.vml[i].shape.appendChild(m.vml[i].fill);
				m.parentNode.insertBefore(m.vml[i].shape, m);
			}
		}
		m.vml.image.shape.fillcolor = "none";
		m.vml.image.fill.type = "tile";
		m.vml.color.fill.on = false;
		j.attachHandlers(m);
		j.giveLayout(m);
		j.giveLayout(m.offsetParent);
		m.vmlInitiated = true;
		j.applyVML(m);
	}
};
...
/* JQ ���� ���� */
(function(b) {
	b.fn.webwidget_menu_glide = function(d) {
		var d = d || {};
		var B = d && d.menu_text_size ? d.menu_text_size: "15";
		var A = d && d.menu_text_color ? d.menu_text_color: "";
		var z = d && d.menu_margin ? d.menu_margin: "10";
		var y = d && d.menu_width ? d.menu_width: "120";
		var x = d && d.menu_height ? d.menu_height: "36";
		var w = d && d.menu_sprite_color ? d.menu_sprite_color: "red";
		var v = d && d.menu_background_color ? d.menu_background_color: "";
		var u = d && d.sprite_speed ? d.sprite_speed: "fast";
		B += "px";
		z += "px";
		y += "px";
		x += "px";
		var t = b(this);
		if (t.children("ul").length == 0 || t.find("li").length == 0) {
			t.append("Require menu content");
			return null;
		}
		s(t.children("ul").children("li"), z, y, x);
		a(t.find("a"), A, x, B);
		t.css("background-color", v);
		if (t.children("ul").children("li").is(".current")) {
			var e = t.children("ul").children("li").filter(".current").position();
		} else {
			var e = t.children("ul").children("li:first").position();
		}
		var c = e.left + "px";
		C(t.find(".webwidget_menu_glide_sprite"), w, y, x, c);
		t.children("ul").children("li").hover(function() {
			var f = $(this).position();
			var g = f.left + "px";
			t.find(".webwidget_menu_glide_sprite").stop().animate({
				left: g
			},
			u);
		},
		...
		t.children("ul").children("li").children("ul").children("li").hover(function() {},
		function() {});
		function a(g, f, i, h) {
			g.css("color", f);
			g.css("font-size", h);
		...
		function s(g, f, i, h) {
			style = " width: " + i + "; height: " + h + ";";
			g.attr("style", style);
		}
		function C(g, f, j, i, h) {
...
$(function() {
	$(".personal_message_close").click(function() {
		$(this).parent().hide();
		var a = $("#userid").val();
		...
		$.getJSON("web/seeTips.htm", c,
		function(d) {});
	});
	$(".person_list p").hover(function() {
	...
	$(".person").mouseover(function() {
		$(".person_list").show();
		$(this).find(".J-arrows").removeClass("down_arrows").addClass("up_arrows");
	}).mouseout(function() {
		$(".person_list").hide();
		$(this).find(".J-arrows").removeClass("up_arrows").addClass("down_arrows");
	});
	$(".person_list li").mousemove(function() {
		$(".person_list").show();
	});
	$(".J-publish").hover(function() {
		$(this).addClass("lvtu_addtravels_hover");
		$(this).removeClass("lvtu_addtravels");
	},
	function() {
		$(this).addClass("lvtu_addtravels");
		$(this).removeClass("lvtu_addtravels_hover");
	});
	if ($("#currIndex").val() == 1) {
		...
		if ($("#currIndex").val() == 2) {
			...
			if ($("#currIndex").val() == 3) {
				$("#daren").addClass("current");
			} else {
				if ($("#currIndex").val() == 4) {
					if ($("#hostId").val() == $("#guestId").val()) {
					...
					if ($("#currIndex").val() == 5) {
						$("#iphone").addClass("current");
					}
				}
			}
		}
	}
	$("#tiles li:not(:first)").hover(function() {
		$(this).removeClass("border-white");
		$(this).find(".lvtu_user").removeClass("bbb");
		$(this).find(".lvtu_counet").removeClass("bbb");
		$(this).find(".lvtu_address").removeClass("darkgray");
		$(this).addClass("border-line");
		$(this).find(".lvtu_user").addClass("darkgray");
		$(this).find(".lvtu_counet").addClass("darkgray");
		$(this).find(".lvtu_address").addClass("darkgray999");
		$(this).find(".lvtu_scenery").attr("style", "background:url(/lvtu/views/default/image/lvtu_home_icon_2_2.png) -2px -464px no-repeat");
		$(this).find(".lvtu_comment").attr("style", "background:url(/lvtu/views/default/image/lvtu_home_icon_2_2.png) -193px -468px no-repeat");
		$(this).find(".lvtu_approve").attr("style", "background:url(/lvtu/views/default/image/lvtu_home_icon_2_2.png) -277px -468px no-repeat");
	},
	function() {
		$(this).removeClass("border-line");
		$(this).find(".lvtu_user").removeClass("darkgray");
		$(this).find(".lvtu_counet").removeClass("darkgray");
		$(this).find(".lvtu_address").removeClass("darkgray999");
		$(this).addClass("border-white");
		$(this).find(".lvtu_user").addClass("bbb");
		$(this).find(".lvtu_counet").addClass("bbb");
		$(this).find(".lvtu_address").addClass("darkgray");
		$(this).find(".lvtu_scenery").attr("style", "background:url(/lvtu/views/default/image/lvtu_home_icon_2_2.png) -2px -223px no-repeat");
		$(this).find(".lvtu_comment").attr("style", "background:url(/lvtu/views/default/image/lvtu_home_icon_2_2.png) -193px -225px no-repeat");
		$(this).find(".lvtu_approve").attr("style", "background:url(/lvtu/views/default/image/lvtu/lvtu_home_icon_2_2.png) -277px -225px no-repeat");
	});
...
function escapeJavascript(a) {
	return String(a).replace(/\\/g, "\\\\").replace(/\t/g, "\\t").replace(/"/g, "\\x22").replace(/'/g, "\\x27").replace(/&/g, "\\x26").replace(/</g, "\\x3C").replace(/>/g, "\\x3E");
}
/**
 * �ض���URL
 * @param string a Ҫ�ض��򵽵ĵ�ַ
 * @param boolean b �Ƿ��ض��򵽱������������� true Ϊ�ض���������
 */
function redirectUrl(a, b) {
	if (b) {
		window.location.href = (is_ssl ? "https" : "http") + "://cn.86.usitrip-1.com/login.php?ret=" + encodeURIComponent(a);
	} else {
		window.location.href = a;
	}
}
function seeTips(b, d) {
	var a = $("#userid").val();
	...
	$("#msg").find("li").eq(b - 1).remove();
	$.getJSON("web/seeTips.htm", e,
	function(f) {
		if (f != null && f.status) {} else {}
	});
}
$(function() {
	...
	$(".lvtu_addtravels,.addtravels").click(function() {/* �ҵ���ͼ�˵��� �����μǵ���¼�  */
	...
	$("#persons,.myhome").click(function() { /*   �ҵ���ҳ����¼�   */
		var a = $("#userid").val();
		if (a == "" || a == "undefined") {
	...
	$(".myfriend").click(function() {
		var a = $("#userid").val();
		if (a != null && a != "") {
			redirectUrl(pathUpload + "web/userFocusList.htm?userId=" + a + "&type=1", false);
		} else {
			redirectUrl(pathUpload + "web/person.htm?type=2", true);
		}
		return false;
	});
	$(".commentandmessage").click(function() {
		var a = $("#userid").val();
		if (a != null && a != "") {
			redirectUrl(pathUpload + "web/commentlist.htm?userId=" + a + "&type=1", false);
		} else {
			redirectUrl(pathUpload + "web/person.htm?type=3", true);
		}
		return false;
	});
	$(".accountset").click(function() {
		var a = $("#userid").val();
		if (a != null && a != "") {
			redirectUrl("http://user.qunar.com/index.jsp#basic", false);
		} else {
			redirectUrl("http://user.qunar.com/index.jsp#basic", true);
		}
		return false;
	});
});