...
var timer;
$(function() {
    $("#editAlbumF").tokenInput(pathUpload + "?module=album&action=searchPOI", {
        theme: "facebook",
        tokenLimit: 1
    });
    $(".J-specialk-btn").bind({
...
function upload1(a, b) {
    $("#id_upload" + a).uploadify({
        ...
        swf: pathUpload + "views/new/image/uploadify.swf",
        uploader: pathUpload + "?module=album&action=upimg",
        buttonImage: pathUpload + "views/new/image/special_add.jpg",
        fileTypeExts: "*.jpg; *.png; *.jpeg;",
        fileObjName: "myfile",
        width: 186,
        queueSizeLimit: 30,
...
function upload(a) {
    $("#id_upload" + a).uploadify({
        ...
        removeCompleted: false,
        onInit: function(b) {
            $("#batch" + a).find("input[name='batchdes']").tokenInput(pathUpload + "?module=album&action=searchPOI", {
                theme: "facebook",
                tokenLimit: 1
            });
            $("#finish").attr("class", "jk_upload_undone ml15 mb15 mr15 fn-tc");
...
$(".J-specialk-btn").click(function() {
    ...
    var l = new Array();
    l = h.split("&");*/
    var k = lwkai.getUrlParam('before'); //before
    var e = lwkai.getUrlParam('after'); //after
    var f = lwkai.getUrlParam('type'); //type
    var o = lwkai.getUrlParam('dayid'); //dayid
    ...
    if (isUploading == true) {
        ...
        if (isUploading == false) {
            ...
            if (f == 1) {
                ...
                $.ajax({
                    type: "POST",
                    url: pathUpload + "?module=album&&action=savePictureOther",
                    data: t,
                    dataType: "json",
                    success: function(i) {
                        if (i != null && i.albumId != null) {
                            ...
                            if (i.id != null) {
                                window.location.href = pathUpload + "album/show/albumId--" + i.albumId + "--dayid--" + o + "--im--" + i.id + ".html";
                            } else {
                                window.location.href = pathUpload + "album/show/albumId--" + i.albumId + "--dayid--" + o + ".html";
                            }
                        } else {}
                    }
                });
