...
// 这里参数 原来就一个A,  B 和 C 都是我加的 by lwkai
SWFUpload.prototype.fileQueued = function(a) {
//SWFUpload.prototype.fileQueued = function(b,a,c) {
...
function addAlbum() {
	...
	$(".addAlbum_newAlbum").show();
}
var newHandler = $("#newHandler").val();
$(function() {
	$(".J-addGuide").click(function() {
		if (newHandler.substring(1, 2) != "0") {
			$(".add_guide_bg,.add_guide_img").hide();
		} else {
			$(".add_guide_bg,.add_guide_img").show();
		}
	});
	$(".add_guide_close").click(function() {
		...
		/*$.getJSON("web/updateHandlerStatus.htm", c,
		function(d) {
			newHandler = d.status;
		});*/
	});
	$(".J-addGuidet").click(function() {
		if (newHandler.substring(2, 3) != "0") {
			$(".add_guide_bg,.addPic_guide_img").hide();
		} else {
			$(".add_guide_bg,.addPic_guide_img").show();
		}
	});
	$(".addPic_guide_close").click(function() {
		$(".add_guide_bg,.addPic_guide_img,.add_guidet_fc").hide();
		var a = $("#userId").val();
		var b = new Date().getTime();
		var c = {
			staus: "0010",
	...
	$("#token-input-addressId").mouseup(function() {});

	$("#albumName").blur(function() { //游记标题失去焦点触发事件
		...
		if ($("#albumName").val() != null && $.trim($("#albumName").val()) != "") {
			$.getJSON(pathUpload + "?module=album&action=check", b,
			function(c) {
				if (c.isReName == true) {
					doubble = false;
				} else {
					doubble = true;
					var d = "<p id='doubleName' class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>专辑已存在，无法创建，请修改标题</span></p>";
					$("#albumName").parent().parent().find(".r").html(d);
				}
			});
		}
	});
...
$(function() {
	$("#albumName").bind("keyup paste keydown",// 旅游记标题 键盘事件 统计字数
	function() {
		var curLength = $.trim($(this).val()).length;
		worldlenName = 40;
		if (curLength > worldlenName) {
			var num = $.trim($(this).val()).length - worldlenName;
			albumNote = "<em class='orange'>" + eval(parseInt(worldlenName) + parseInt(num)) + "</em>字/" + worldlenName + "字,您已超过<em class='orange'>" + num + "</em>字";
			...
			if (curLength == 0) {
				if ($.trim($(this).val()) == "") {
					$(this).parent().parent().find(".r").html("<p id='doubleName' ><span>还可以输入<em >" + worldlenName + "</em>个字</span></p>");
				} else {
					$(this).parent().parent().find(".r").html("<p id='doubleName'><span>还可以输入<em >" + worldlenName - 1 + "</em>个字</span></p>");
				}
			} else {
				var num = worldlenName - $.trim($(this).val()).length;
				$(this).parent().parent().find(".r").html("<p id='doubleName' ><span>还可以输入<em >" + num + "</em>个字</span></p>");
			}
		}
		setState();
	});
	$("#description").bind("keyup paste keydown",// 描述还可以输入多少字
	function() {
		var curLength = $.trim($(this).val()).length;
		worldlenDesc = 140;
		if (curLength > worldlenDesc) {
			var num = $.trim($(this).val()).length - worldlenDesc;
			descNote = "<em class='orange'>" + eval(parseInt(worldlenDesc) + parseInt(num)) + "</em>字/" + worldlenDesc + "字,您已超过<em class='orange'>" + num + "</em>字";
			$(this).parent().parent().find(".r").html("<p id ='albumDesc' class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>" + descNote + "</span></p>");
		} else {
			if (curLength == 0) {
				if ($.trim($(this).val()) == "") {
					$(this).parent().parent().find(".r").html("<p id='doubleName' ><span>还可以输入<em >" + worldlenDesc + "</em>个字</span></p>");
				} else {
					$(this).parent().parent().find(".r").html("<p id='doubleName' ><span>还可以输入<em >" + worldlenDesc - 1 + "</em>个字</span></p>");
				}
			} else {
				var num = worldlenDesc - $.trim($(this).val()).length;
				$(this).parent().parent().find(".r").html("<p id='doubleName'><span>还可以输入<em >" + num + "</em>个字</span></p>");
			}
		}
		setState();
...
$(document).ready(function() {
	$("#id_upload").uploadify({
		debug: false,
		height: 33,
		swf: pathUpload + "views/new/image/uploadify.swf",//SWFUpload.swf",
		...
		onUploadSuccess: function(b, d, c) {
			...
			if (d == null) {
				...
				function(j, k) {
					...
					e += "<div class='img_textarea'><textarea class='border-ca gray' name='desc' cols='' rows=''>图片描述（可不填写）</textarea></div>";
	...
	//$("#addressId").tokenInput("web/searchDestin.htm", {
	$("#addressId").tokenInput(pathUpload + "?module=album&action=searchDestin",{
		searchDelay: 0,
		animateDropdown: false,
		theme: "facebook"
	});
	$("#psAddressId").tokenInput(pathUpload + "?module=album&action=searchPOI", {
		theme: "facebook",
		tokenLimit: 1
	});
	$('#trip_line').tokenInput(pathUpload + "?module=album&action=searchTrip",{
		theme: "facebook",
		animateDropdown: false,
		searchDelay: 0,
		tokenLimit: 1
...
function upload1(a, b) {
	$("#id_upload" + a).uploadify({
		height: 124,
		swf: pathUpload + "views/default/image/uploadify.swf",//SWFUpload.swf",
		uploader: pathUpload + "?module=album&action=upimg",
		buttonImage: "lvtu/views/new/image/special_add.jpg",
		fileTypeExts: "*.jpg; *.png; *.jpeg;",
		fileObjName: "myfile",
		width: 186,
		queueSizeLimit: 30,
				...
				if (a == 0) {
					if (c == false) {
						...
						if (c == true) {
							...
							g += "<span class='close jk_close'></span>";
...
function upload(a) {
	$("#id_upload" + a).uploadify({
		height: 30,
		swf: pathUpload + "views/default/image/uploadify.swf",//SWFUpload.swf",
		uploader:  pathUpload + "?module=album&action=upimg", //第二个地方的上传图
		buttonImage: pathUpload + "views/new/image/specialadd_next.jpg",
		fileTypeExts: "*.jpg; *.png; *.jpeg;",
		fileObjName: "myfile",
		debug:false,
		width: 126,
		...
		removeCompleted: false,
		onInit: function(b) {
			$("#batch" + a).find("input[name='batchdes']").tokenInput(pathUpload + "?module=album&action=searchPOI&b=2", {
				theme: "facebook",
				tokenLimit: 1
			});
			$("#finish").attr("class", "jk_upload_undone ml15 mb15 mr15 fn-tc");
...
function close() {
	...
	$(".cover").unbind("click"); //去掉封面按钮的CLICK事件
	$(".cover").click(function() { //重新绑上CLICK事件
		$(".lvtu_special_cover").removeClass("lvtu_special_cover");
		$(this).parent().parent().find("span").eq(0).toggleClass("lvtu_special_cover png24");
	});
	$(".rotate").unbind("click");
	...
	$("#imageId ul li textarea").each(function() {
		...
		function() {
			...
			if (curLength > worldlen) {
				...
				$(this).parent().parent().find(".lvtu_special_text").html("<div class='lvtu_special_text_bg fn-fl'>(<em class='orange'>" + eval(parseInt(worldlen) + parseInt(num)) + "</em>字/" + worldlen + "字,您已超过<em class='orange'>" + num + "</em>字)</div>");
			} else {
				if (curLength == 0) {
					if ($.trim($(this).val()) == "") {
...
function uploadFilter() {
	$(".testaaa").tokenInput(pathUpload + "?module=album&action=searchPOI", {
		theme: "facebook",
		tokenLimit: 1
	});
	$(".lvtu_special_pic li:not(:last)").hover(function() {
		$(this).addClass("background-e");
		$(this).find(".lvtu_special_tips,.lvtu_special_service").fadeIn();
	},
	function() {
		$(this).removeClass("background-e");
		$(this).find(".lvtu_special_tips,.lvtu_special_service").fadeOut();
	});
	...
	$(".lvtu_special_service span a").click(function() {
		...
		$(this).removeClass("service_hover");
		$(this).addClass("service_down").parent().siblings().children().removeClass("service_down").addClass("service_link");
	});
	$(".lvtu_special_service span a").hover(function() {
		if ($(this).hasClass("service_down")) {} else {
			$(this).removeClass("service_link");
			$(this).addClass("service_hover");
		}
	},
	function() {
		$(this).removeClass("service_hover");
		$(this).addClass("service_link");
	});
	$(".J-special-btn").bind({
		mouseover: function() {
			$(".J-special-btn").addClass("special_btn_out").removeClass("special_btn special_btn_down");
		},
		mouseout: function() {
			$(".J-special-btn").addClass("special_btn").removeClass("special_btn_out special_btn_down");
		},
		mousedown: function() {
			$(".J-special-btn").addClass("special_btn_down").removeClass("special_btn special_btn_out");
		}
	});
}
function setState() {
...
/* 保存游记 */
$(".lvtu_special_button_create").click(function() {
	if (isUploading == true) {
		...
		if (isUploading == false) {
			...
			c = c.replace(/<\/?[^>]*>/g, "");
			var m = "";
			var trip_line = $('#trip_line').tokenInput("get");
			var trip_line = getAddressStr(trip_line);
			var s = $("#addressId").tokenInput("get");
			var r = getAddressStr(s);
			var j = $("#batch0 input[name='batchdes']").tokenInput("get");
			var k = getAddressStr(j);
			var b = true;
			var v = new Array();
			if (c == null || c == "") {
				var q = "<p id='doubleName' class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>* 游记名称不能为空</span></p>";
				$("#albumName").parent().parent().find(".r").html(q);
				v[0] = "#albumName";
				b = false;
			}
			/*校验旅游线路是否填写
			if (trip_line == null || trip_line == "" || trip_line == '!') {
				var l1 = "<p class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>请选择此行的旅游线路</span></p>";
				$("#trip_line").parent().parent().find(".r").html(l1);
				v[2] = "#trip_line";
				b = false;
			}
			*/
			if (doubble == true) {
				var o = "<p id='doubleName' class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>专辑已存在，无法创建，请修改标题</span></p>";
				$("#albumName").parent().parent().find(".r").html(o);
				v[0] = "#albumName";
				b = false;
			}
			if (c.length > 40) {
				var u = "<p id='doubleName' class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>" + albumNote + "</span></p>";
				$("#albumName").parent().parent().find(".r").html(u);
				v[0] = "#albumName";
				b = false;
			}
			if (r == "!" || r == "") {
				var l = "<p class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>* 请选择此行的目的地</span></p>";
				$("#addressId").parent().parent().find(".r").html(l);
				v[1] = "#addressId";
				b = false;
			}
			if ($("#query0").html() == "") {
				var t = "<p class='specialadd_talk_error f12 darkgray666 png24'><span class='png24'>* 请至少添加一张图片</span></p>";
				$("#uploadFlash").parent().find(".r").empty();
				$("#uploadFlash").parent().find(".r").html(t);
			}
			if (b == false) {
				$(v[0]).focus();
			...
			var j = new Array();
			var picNum = 0; // lwkai add 统计有多少个图片
			if (b == true) {
				$.jBox.tip("正在提交游记信息...", "loading");
				var g = "<xml>";
				g += "      <userId>255</userId>";
				g += "      <name>" + c + "</name>";// 游记标题
				g += "      <description>" + m + "</description>";//描述 目前没有用，描述改在后面再添加了，添加图片的时候不添加描述
				g += "      <destination>" + r + "</destination>";//多个目的地
				g += "      <coverPic>" + r + "</coverPic>";//覆盖图片?? 这个不知道具体用途
				g += "      <tripLine>" + trip_line + "</tripLine>"; //旅游线路
				g += "      <batchList>";
				g += "          <batch>";
				g += "               <batchdes>" + k + "</batchdes>";//拍摄地点 只能选一个
				g += "               <batchPicList>";
				$("#imageId  ul  li").each(function() {
					var y = $(this).find("#imgget").attr("name");
					var D = $(this).find("textarea[name='desc']").val(); 
					var A = $(this).find("input[name='camera_time']").val();
					var B = $(this).find("input[name='camera_model']").val();
					var E = $(this).find("input[name='fnumber']").val();
					var H = $(this).find("input[name='camera_iso']").val();
					var I = $(this).find("input[name='exposure_time']").val();
					var z = $(this).find("input[name='exposure_bias']").val();
					var x = $(this).find("input[name='focal_length']").val();
					var w = $(this).find("input[name='picWidth']").val();
					var C = $(this).find("input[name='picLength']").val();
					var i = $(this).find("input[name='lon']").val();
					var F = $(this).find("input[name='lat']").val();
					var G = $(this).find(".service_down").attr("id");
					var J = $(this).find(".lvtu_special_cover").html();
					B = msgEscape(B);
					H = msgEscape(H);
					if (D == "图片描述（可不填写）") {
						D = "";
					...
					if (y != null) {
						...
						g += "                        <desc>" + D + "</desc>"; // 图片描述
						g += "                        <isCover>" + J + "</isCover>";//是否封面
						g += "                        <picWidth>" + w + "</picWidth>";//图片宽度
						g += "                        <picLength>" + C + "</picLength>";//图片高度
						g += "                        <exif>";
						g += "                       <Camera_Time>" + A + "</Camera_Time>";//拍摄时间
						g += "                        <Camera_Model>" + B + "</Camera_Model>";//相机型号
						g += "                        <Fnumber>" + E + "</Fnumber>";
						g += "                        <Camera_ISO>" + H + "</Camera_ISO>";
						g += "                       <Exposure_Time>" + I + "</Exposure_Time>";
						g += "                       <Exposure_Bias>" + z + "</Exposure_Bias>";
						g += "                        <Focal_Length>" + x + "</Focal_Length>";
						g += "                        <GpsLongitude>" + i + "</GpsLongitude>";
						g += "                        <GpsLatitude>" + F + "</GpsLatitude>";
						g += "                        </exif>";
						g += "                   </batchPic>";
						picNum ++;
					};
				...
				for (var n = 1; n < templateId; n++) {
					...
					var e = getAddressStr(p);
					g += "          <batch>";
					g += "               <batchdes>" + e + "</batchdes>";
					g += "               <batchPicList>";
					$("#batch" + n + " ul li").each(function() {
						...
						var E = $(this).find("input[name='fnumber']").val();
						var H = $(this).find("input[name='camera_iso']").val();
						var J = $(this).find("input[name='exposure_time']").val();
						var z = $(this).find("input[name='exposure_bias']").val();
						var x = $(this).find("input[name='focal_length']").val();
						var C = $(this).find("input[name='picLength']").val();
						var w = $(this).find("input[name='picWidth']").val();
						...
						var F = $(this).find("input[name='lat']").val();
						var G = $(this).find(".service_down").attr("id");
						var I = $(this).find("textarea[name='exif']").val();
						B = msgEscape(B);
						H = msgEscape(H);
						var K = $(this).find(".lvtu_special_cover").html();
						if (K != null) {
							K = true;
							h = true;
						} else {
						...
						if (y != null) {
							...
							g += "                        <desc>" + D + "</desc>";
							g += "                        <isCover>" + K + "</isCover>";
							g += "                        <picWidth>" + w + "</picWidth>";
							g += "                        <picLength>" + C + "</picLength>";
							g += "                        <exif>";
							g += "                       <Camera_Time>" + A + "</Camera_Time>";
							g += "                        <Camera_Model>" + B + "</Camera_Model>";
							g += "                        <Fnumber>" + E + "</Fnumber>";
							g += "                        <Camera_ISO>" + H + "</Camera_ISO>";
							g += "                       <Exposure_Time>" + J + "</Exposure_Time>";
							g += "                       <Exposure_Bias>" + z + "</Exposure_Bias>";
							g += "                        <Focal_Length>" + x + "</Focal_Length>";
							g += "                        <GpsLongitude>" + i + "</GpsLongitude>";
							g += "                        <GpsLatitude>" + F + "</GpsLatitude>";
				...
				g += "       </batchList>";
				g += "       <picNum>" + picNum + "</picNum>";
				g += "</xml>";
				if (h == false) {
					$.jBox.tip("请至少添加一个封面", "warning");
					return false;
				}
				if (d == true) {
				...
				$.ajax({
					type: "POST",
					url: pathUpload + "?module=album&action=save", //保存数据的请求URL
					data: f,
					dataType: "json",
					success: function(i) {
						if (i != null && i.albumId != null) {
							$.jBox.tip("添加游记成功", "success");
							window.location.href = pathUpload + "album/show/albumId--" + i.albumId + ".html";
						} else {
							$.jBox.tip("添加游记失败\n" + i.msg, "warning");
						}
					}
...
$(function() {
	$("#token-input-addressId,#token-input-trip_line").focus(function() {
