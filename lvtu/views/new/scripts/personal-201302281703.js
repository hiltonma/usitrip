...
jQuery(document).ready(function(a) {
	a("img.qpic").lazyload({
		placeholder: lwkai.imageUrl + "lvtu_bg_2.jpg",//"http://source.qunar.com/site/images/wap/lvtu/lvtu_bg_2.jpg",
		effect: showeffect,
		failurelimit: 2
	});
}); (function(b, a, c) {
...
$(function() {
	...
	$(".J-img").hover(function() {
		...
		$(this).find(".J-imgNum").stop().show().animate({opacity:0.8},500);//fadeIn("slow");//.css("left", e);
	},
	function() {
		/*$(this).find(".personal_pic_bg").stop().animate({
			bottom: "-400px"
		});*/
		$(this).find(".J-imgNum").stop().show().animate({opacity:0},200);//fadeOut("slow");
	});
	$(function() {
		$("img.lazy").lazyload({
			event: "scrollstop",
	...
	$("#pagination").paging(c, {
		...
		onSelect: function(f) {
			if ($("#isFirstCome").val() == 1) {
				...
				var e = $("#hostId").val();
				window.location.href = pathUpload + "album/userinfo/userId--" + e + "--page--" + f + ".html";
			}
		}
	});
});
$(function() {
	function a() {
		...
		var f = $('#foot').offset().top-$(".personal_sidebar").height()-80;
