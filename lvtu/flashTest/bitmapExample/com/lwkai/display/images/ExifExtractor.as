package com.lwkai.display.images {
	...
	public class ExifExtractor {
		...
		public function ExifExtractor(target:FileReference,testObj:SWFUpload):void {
			...
			for (i = 0, len = this.IFDDirectory.length; i < len; i++) {
				switch (this.IFDDirectory[i][IFD_IDX_Tag_Name]['en']) {
					case "Orientation":
						...
						this.testObj.Debug('Orientation this.IFDDirectory[' + i + '][IFD_IDX_Value]=' + this.IFDDirectory[i][IFD_IDX_Value]);
					...
					case "Exposure Bias Value":
						...
						if (TagValues.length - 1 == 1) {
							if (parseFloat(TagValues[1]) != 0) {
								...
								if (TagValues[0] != 0) {
									if (parseFloat(Math.abs(TagValues[0])) < parseFloat(Math.abs(TagValues[1])) && parseFloat(TagValues[0]) != 0) {
										this.IFDDirectory[i][IFD_IDX_Value_Desc] = this.IFDDirectory[i][IFD_IDX_Value_Desc] + "0";
									}
									this.IFDDirectory[i][IFD_IDX_Value_Desc] = this.IFDDirectory[i][IFD_IDX_Value_Desc] + (parseFloat(Math.abs(TagValues[0])) / parseFloat(Math.abs(TagValues[1]))).toFixed(1);
								}
